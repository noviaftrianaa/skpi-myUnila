    ...
    public function index(Request $request)
    {
        ...
        if ($unit->id_jns_lemb==24) {
            ...
            $ta_list = TahunAjaran::select('id_thn_ajaran','nm_thn_ajaran')
              ...
              ->toArray();
            $judul = 'Tenaga Kependidikan Fakultas '.$sms->fakultas_unila->nm_lemb.' / Prodi '.$sms->nm_lemb.' ('.$sms->jenjang->nm_jenj_didik.')';
            $unit_filter = 'Fakultas '.$sms->fakultas_unila->nm_lemb;
        } elseif ($unit->id_jns_lemb==28) {
            $sms = Sms::find($unit->id_organisasi);
            $judul = 'Tenaga Kependidikan Fakultas '.$sms->fakultas_unila->nm_lemb.' / Jurusan'.$sms->nm_lemb;
            $unit_filter = 'Fakultas '.$sms->fakultas_unila->nm_lemb;
        ...
        $data = SDM::get_data_all_tendik($thn,$unit->level_organisasi,$unit_filter);
        return view('content.main.sdm.tendik.index',compact('thn','ta_list','judul','data'));
    }
}
