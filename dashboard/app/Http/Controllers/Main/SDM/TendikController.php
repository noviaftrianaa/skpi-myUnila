...
class TendikController extends Controller
{
  public function index(Request $request)
  {
    ...
    return view('content.main.sdm.tendik.index', compact('thn', 'ta_list', 'judul', 'unit', 'unit_filter'));
  }

  public function data(Request $request)
  {
    $data = SDM::get_data_all_tendik($request->tahun, $request->level_organisasi, $request->unit_filter);

    return \DataTables::of($data)
      ->addIndexColumn()
      ->addColumn('usia', function ($data) {
        return tglIndonesiaShort($data->tgl_lahir) . ' (' . $data->umur . ' Tahun)';
      })
      ->rawColumns(['usia'])
      ->make(true);
  }
