<script type="text/javascript">
    ...
    function DownloadAllRaw(id_jns_sms, id) {
        ...
        var url = "{{ route('download-raw-iku5') }}?thn_iku=" + thn_iku + "&id_jns_sms=" + id_jns_sms + "&id_sms=" + id;
        location.href = url;
    }
        ...
        if (dt_point_iku.length) {
            ...
            $.ajax({
                ...
                "success": function(data) {
                    ...
                    $('#total_karya').text(data.count.total_publikasi);
    ...
    function TableRawIku(id, prodi) {
        ...
        var dt_raw_iku5 = $('.datatables-raw-iku5');
            dt_raw_iku5;
        var data = [];
        var id_jns_sms = 3;

        if (dt_raw_iku5.length) {
            $('#loading_raw_table').show();
            $.ajax({
                url: '{!! route('json-raw-iku5') !!}',
                type: "GET",
                datatype: 'json',
                data: {
                    thn_iku: $("#thn_iku").val(),
                ...
                "success": function(data) {
                    ...
                    dt_raw_iku5 = dt_raw_iku5.DataTable({
                            bDestroy: true,
                            data: data,
                            columns:[
                                { data: 'id_sdm' },
                                ...
                                { data: 'total_publikasi_semua', title: 'Total Karya Dosen' },
                                { data: 'total_publikasi_tertinggi', title: 'Total Terverifikasi' },
                                { data: 'total_bobot', title: 'Total Total Point' },
                            ],
                            columnDefs: [
                                {
                                    target: 0,
        ...
        if ($.fn.DataTable.isDataTable('.datatables-raw-iku5')) {
            $('.datatables-raw-iku5').DataTable().destroy();
        }

    };

    function ClearCloseTable() {
        $('.datatables-raw-iku5').DataTable().clear().draw();
    }
</script>
