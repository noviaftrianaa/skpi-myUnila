<script type="text/javascript">
    ...
    function DownloadAllRaw(id_jns_sms, id) {
        ...
        var url = "{{ route('download-raw-iku6') }}?thn_iku=" + thn_iku + "&id_jns_sms=" + id_jns_sms + "&id_sms=" + id;
        location.href = url;
    }
        ...
        if (dt_point_iku.length) {
            ...
            $.ajax({
                ...
                "success": function(data) {
                    ...
                    $('#total_valid').text(data.count.total_valid);
                    $('#total_prodi').text(data.count.total_prodi);
    ...
    function TableRawIku(id, prodi) {
        ...
        var dt_raw_iku6 = $('.datatables-raw-iku6');
            dt_raw_iku6;
        var data = [];
        var id_jns_sms = 3;

        if (dt_raw_iku6.length) {
            $('#loading_raw_table').show();
            $.ajax({
                url: '{!! route('json-raw-iku6') !!}',
                type: "GET",
                datatype: 'json',
                data: {
                    thn_iku: $("#thn_iku").val(),
                ...
                "success": function(data) {
                    ...
                    dt_raw_iku6 = dt_raw_iku6.DataTable({
                            bDestroy: true,
                            data: data,
                            columns:[
                                { data: 'id_sms' },
                                { data: 'nm_fak', title: 'Fakultas', },
                                { data: 'nm_prodi',  title: 'Prodi', },
                                { data: 'nm_jenj_didik', title: 'Jenjang' },
                                { data: 'nm_dudi', title: 'Nama Mitra' },
                                { data: 'nm_kriteria_mitra', title: 'Kriteria Mitra' },
                                { data: 'nm_akt_kerjasama', title: 'Aktivitas Kerjasama' },
                                { data: 'tgl_mulai', title: 'Tanggal Mulai' },
                                { data: 'tgl_selesai', title: 'Tanggal Selesai' },
                                { data: 'nm_bntk_giat_kerjasama', title: 'Bentuk Kegiatan' },
                                { data: 'judul_mou', title: 'Judul Mou' },
                                { data: 'verifikasi', title: 'Verifikasi PDIKTI' },
                            ...
                            columnDefs: [
                                ...
                                },
                                {
                                    ...
                                    width: 30,
                                    render: function(data, type, row) {
                                       return `<a href="{{ route('pages-prodi', '') }}/${row.encrypted_id_sms}" target="_blank">${data}</a>`;
                                    }
                                },
                                {
                                    target: 3,
        ...
        if ($.fn.DataTable.isDataTable('.datatables-raw-iku6')) {
            $('.datatables-raw-iku6').DataTable().destroy();
        }

    };

    function ClearCloseTable() {
        $('.datatables-raw-iku6').DataTable().clear().draw();
    }
</script>
