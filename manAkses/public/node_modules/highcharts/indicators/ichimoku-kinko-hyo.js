...
arguments);return d},getValues:function(a,d){var b=d.period,e=d.periodTenkan;d=d.periodSenkouSpanB;var l=a.xData,h=a.yData,m=h&&h.length||0;a=F(a.xAxis);var g=[],n=[],c;if(!(l.length<=b)&&H(h[0])&&4===h[0].length){var u=l[0]-b*a;for(c=0;c<b;c++)n.push(u+c*a);for(c=0;c<m;c++){if(c>=e){var f=h.slice(c-e,c);f=z(f);f=(f.high+f.low)/2}if(c>=b){var v=h.slice(c-b,c);v=z(v);v=(v.high+v.low)/2;var w=(f+v)/2}if(c>=d){var q=h.slice(c-d,c);q=z(q);q=(q.high+q.low)/2}u=h[c][3];var x=l[c];g[c]===p&&(g[c]=[]);g[c+
b]===p&&(g[c+b]=[]);g[c+b][0]=f;g[c+b][1]=v;g[c+b][2]=p;g[c][2]=u;c<=b&&(g[c+b][3]=p,g[c+b][4]=p);g[c+2*b]===p&&(g[c+2*b]=[]);g[c+2*b][3]=w;g[c+2*b][4]=q;n.push(x)}for(c=1;c<=b;c++)n.push(x+c*a);return{values:g,xData:n,yData:g}}}});""});l(d,"masters/indicators/ichimoku-kinko-hyo.src.js",[],function(){})});
