...
/**
 * SSO to man_akses Sync Job
 *
 * Sync data dari MySQL radcheck (SSO) ke SQL Server man_akses.pengguna
 * dengan SHA1 Rehashing strategy untuk password security
 */
class SyncSSOToManAksesJob implements ShouldQueue
{
     ...
     */
    public function __construct(int $chunkSize = 500, int $progressInterval = 10)
    {
    ...
    {
        Log::info('[SSO Sync Job] Starting', [
            'job_id' => $this->jobId,
        ...
        try {
            ...
            $this->totalUsers = DB::connection('mysql')
                ->table('radcheck')
                ->where('a_aktif', 1)
                ->where('soft_delete', 0)
            ...
            Log::info('[SSO Sync Job] Total users to sync', [
            ...
            Log::info('[SSO Sync Job] Completed', [
                'job_id' => $this->jobId,
                'total' => $this->totalUsers,
    ...
    protected function preloadFakultasDomains(): void
    {
        ...
        Log::info('[SSO Sync Job] Fakultas domains loaded', [
    ...
    protected function processUsersInChunks(): void
    {
        ...
        while (true) {
            ...
            $totalProgress = round(($chunkEnd / $this->totalUsers) * 100, 1);

            Log::info('[SSO Sync Job] Processing chunk', [
                'chunk' => $chunkNumber,
                ...
                foreach ($batch as $user) {
                    try {
                        ...
                        if ($processed % $this->progressInterval == 0 || $processed == $chunkTotal) {
                            $percentage = round(($this->processedUsers / $this->totalUsers) * 100, 2);
                            ...
                            Log::info('[SSO Sync Job] Progress', [
                        ...
                        $this->failedCount++;
                        $processed++;
                        Log::error('[SSO Sync Job] User sync failed', [
    ...
    protected function determineRole(object $user): array
    {
        ...
        if ($domain === 'students') {
            ...
            if (!empty($user->nip) && isset($this->mahasiswaCache[$user->nip])) {
                ...
                Log::warning('[SSO Sync Job] Mahasiswa not found in reg_pd', [
        ...
        elseif ($domain === 'staff') {
            ...
            if (!empty($user->nip) && isset($this->tendikCache[$user->nip])) {
                ...
                Log::warning('[SSO Sync Job] Tendik not found in sikep.pegawai', [
                    'username' => $user->username,
                    'nip' => $user->nip
                ]);
            }
        ...
        elseif (isset($this->fakultasCache[$domain])) {
            ...
            if (!empty($user->nip) && isset($this->dosenCache[$user->nip])) {
                ...
                Log::warning('[SSO Sync Job] Dosen not found in pdrd.sdm', [
                    'username' => $user->username,
                    'nip' => $user->nip,
                    'domain' => $domain
                ]);
        ...
        // GUEST: Domain lain atau tidak dikenali
            ...
            Log::info('[SSO Sync Job] User set to Guest (unknown domain)', [
                'username' => $user->username,
                'email' => $user->email,
                'domain' => $domain
            ]);
    ...
    {
        Log::error('[SSO Sync Job] Job failed permanently', [
            'job_id' => $this->jobId,
