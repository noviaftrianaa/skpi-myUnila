...
--
--   Versi ini = v1.0 + semua ALTER yang sudah diterapkan.
--   Cocok untuk fresh install (skip menjalankan ALTER terpisah).
--
--   CHANGELOG dari v1.0 (per 2026-04-25):
--     - Fix trigger fn_catat_aktivitas_data — pakai TG_ARGV[0] dinamis
--       (ganti hardcoded id_pengajuan fallback).
--     - Pre-seed: PM-ALIH dari 6 ke 5 tahapan (lihat seed-staging).
--     - + ref.ktw_exclude_jalur (KTW exclusion table) + 5 default seed
--     - Trigger updated_at + aktivitas_data juga aktif untuk tabel baru
--
--   Schemas:
--     - ref      : Data referensi/master (jenis layanan, persyaratan, tahapan, template, ktw_exclude_jalur)
...
--     ref.ktw_exclude_jalur        - [v1.1] Jalur pendaftaran yang di-exclude dari KTW
...
-- =====================================================
-- Daftar jalur pendaftaran (siakadu.mahasiswa.jalur_pendaftaran) yang
-- di-exclude dari perhitungan KTW (Kelulusan Tepat Waktu) di monitoring.
-- Contoh: Pindahan/Transfer, RPL, Mahasiswa Asing — masa studi mereka
-- tidak fair dibandingkan jalur reguler.
...
-- Seed default exclusions (5 jalur)
...
--   Total tabel   : 16 (5 ref + 6 layanan + 3 batch + 2 log)
--   Total FK      : 16 (13 inline + 3 deferred ALTER TABLE)
--   Total index   : 42
--   Total trigger : 26 (12 updated_at + 14 aktivitas_data)
--
-- Kolom Audit:
