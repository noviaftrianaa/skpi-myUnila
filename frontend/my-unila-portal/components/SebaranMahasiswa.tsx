...
export default function SebaranMahasiswa() {
  ...
  // Data sebaran mahasiswa per provinsi (sample data)
  const sebaranData = [
    ...
    series: [
      {
        ...
        label: {
          show: true,
          position: "right",
          formatter: (params: any) => `${((params.value / 25000) * 100).toFixed(1)}%`,
          color: "#1f2937",
          fontSize: 12,
          fontWeight: 600,
        },
        ...
        >
          ...
          <motion.div variants={itemVariants} className="text-center mb-16">
            ...
            <p className="text-gray-600 text-lg max-w-2xl mx-auto">
              Distribusi mahasiswa Universitas Lampung berdasarkan asal daerah
            </p>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: "Mahasiswa Lokal", value: "74%", icon: "🏠", color: "blue" },
              { label: "Mahasiswa Luar Daerah", value: "26%", icon: "✈️", color: "indigo" },
              { label: "Provinsi Asal", value: "15+", icon: "🗺️", color: "purple" },
              { label: "Diversity Index", value: "High", icon: "🌟", color: "pink" },
            ].map((stat, index) => (
