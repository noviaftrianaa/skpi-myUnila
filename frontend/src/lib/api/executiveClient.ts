/**
 * Executive API Client with Axios
 *
 * Features:
 ...
 * - Error handling
 *
 * This client is used for Executive Service API calls (Pimpinan Dashboard)
 */

import axios, { AxiosError, AxiosInstance, InternalAxiosRequestConfig } from 'axios';
import { getToken, setToken, clearTokens } from './client';

// Executive API URL via Kong Gateway
// Note: Environment variable should include full path with /api/v1 suffix
const EXECUTIVE_API_URL = process.env.NEXT_PUBLIC_EXECUTIVE_API_URL || 'http://localhost:8087/api/v1';
const API_TIMEOUT = parseInt(process.env.NEXT_PUBLIC_API_TIMEOUT || '30000');

/**
 * Create Executive API Client Instance
 */
const createExecutiveClient = (): AxiosInstance => {
  const instance = axios.create({
    baseURL: EXECUTIVE_API_URL,
    timeout: API_TIMEOUT,
    headers: {
...
/**
 * Executive API Client Instance
 */
export const executiveClient = createExecutiveClient();
