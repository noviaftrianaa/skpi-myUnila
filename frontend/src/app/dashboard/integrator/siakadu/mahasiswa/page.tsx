...
import siakaduService, { SiakaduMahasiswa } from "@/lib/services/siakadu/siakaduService";
...
export default function SiakaduMahasiswaPage() {
  ...
  const handleConfirmSync = async () => {
    ...
    try {
      ...
      const response = await siakaduService.syncMahasiswa(undefined, user?.name || "system");
      ...
      if (response.success) {
        ...
        toast.success("Sinkronisasi mahasiswa berhasil!");
      } else {
      ...
      setSyncResult({ totalRecords: 0, message: errorMessage });
      setTimeout(() => {
        setShowProgressModal(false);
        setSyncProgress(0);
        setSyncStatus("idle");
  ...
  const columns: Column<SiakaduMahasiswa>[] = [
    ...
    { key: "angkatan", label: "Angkatan", sortable: true, align: "center" },
    {
      ...
      label: "IPK",
      align: "center",
      render: (item) => item.ipk?.toFixed(2) || "-",
    },
    {
      key: "status",
      label: "Status",
      ...
      render: (item) => (
        <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
          item.status === "Aktif" || item.status === "A"
              ...
              <ModalHeader className="flex flex-col gap-1 border-b border-gray-200 dark:border-gray-700">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 font-normal">Data Mahasiswa SIAKADU</p>
                  </div>
                </div>
              </ModalHeader>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-sm text-gray-700 dark:text-gray-300 mb-2">
                          Proses ini akan mengambil data mahasiswa terbaru dari SIAKADU dan menyimpannya ke database.
                        </p>
            ...
            <div className="flex items-center gap-3">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-white shadow-lg ${
                ...
                {syncStatus === "success" ? <FiCheckCircle className="w-6 h-6" />
                  : syncStatus === "error" ? <FiXCircle className="w-6 h-6" />
