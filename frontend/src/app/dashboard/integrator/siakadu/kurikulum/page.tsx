...
import siakaduService, { SiakaduKurikulum } from "@/lib/services/siakadu/siakaduService";
...
import { FiBookOpen, FiRefreshCw, FiCheckCircle, FiXCircle, FiClock, FiAlertCircle } from "react-icons/fi";
...
export default function SiakaduKurikulumPage() {
  ...
  const fetchStats = useCallback(async () => {
    try { setIsLoadingStats(true); const r = await siakaduService.getKurikulumStats(); if (r.success) setStats(r.data); } catch (e) { console.error(e); } finally { setIsLoadingStats(false); }
  }, []);

  const fetchData = useCallback(async () => {
    try {
      setIsLoadingData(true);
      const r = await siakaduService.getKurikulumList({ page: currentPage, limit: rowsPerPage, search: searchQuery || undefined });
  ...
  const handleConfirmSync = async () => {
    ...
    try {
      ...
      const r = await siakaduService.syncKurikulum(undefined, user?.name || "system");
      clearInterval(pi);
      if (r.success) {
  ...
  return (
    <DashboardLayoutWithDynamicMenu appName="MyUnila Integrator" appIcon={<MdSchool className="w-6 h-6 text-white" />}
      appKey={APP_KEY} fallbackMenus={myunilaIntegratorMenuConfig} pageTitle="Data Kurikulum SIAKADU">
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Data Kurikulum</h1>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Sinkronisasi data kurikulum dari SIAKADU</p>
          </div>
        ...
        <Card className="border-none shadow-lg rounded-xl overflow-hidden">
          <CardBody className="p-0">
            <DataTable<SiakaduKurikulum> data={data} columns={columns} searchable searchPlaceholder="Cari kode/nama mata kuliah..."
        ...
        <ModalContent>{(onClose) => (<>
          <ModalHeader className="flex flex-col gap-1 border-b border-gray-200 dark:border-gray-700">
            <div className="flex items-center gap-3">
              ...
              <div><h3 className="text-xl font-bold text-gray-800 dark:text-white">Konfirmasi Sinkronisasi</h3><p className="text-sm text-gray-500 dark:text-gray-400 font-normal">Data Kurikulum SIAKADU</p></div>
            </div>
          </ModalHeader>
          <ModalBody className="py-6">
            <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm text-gray-700 dark:text-gray-300 mb-2">Proses ini akan mengambil data kurikulum terbaru dari SIAKADU.</p>
