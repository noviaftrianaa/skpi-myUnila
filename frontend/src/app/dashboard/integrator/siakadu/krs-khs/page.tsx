...
import siakaduService, { SiakaduKRS, SiakaduKHS } from "@/lib/services/siakadu/siakaduService";
import { myunilaIntegratorMenuConfig } from "../../config/menuConfig";

const APP_KEY = "myunila-integrator";

import {
  Card, CardBody, Button, Spinner, Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, Progress, Input, Tabs, Tab,
} from "@heroui/react";
import { FiFileText, FiRefreshCw, FiCheckCircle, FiXCircle, FiClock, FiAlertCircle, FiFilter } from "react-icons/fi";
...
export default function SiakaduKrsKhsPage() {
  ...
  const [activeTab, setActiveTab] = useState<string>("krs");
  ...
  const [khsSemester, setKhsSemester] = useState("");
  ...
  const fetchKrsStats = useCallback(async () => {
    try { setIsLoadingKrsStats(true); const r = await siakaduService.getKRSStats(); if (r.success) setKrsStats(r.data); } catch (e) { console.error(e); } finally { setIsLoadingKrsStats(false); }
  }, []);

  const fetchKhsStats = useCallback(async () => {
    try { setIsLoadingKhsStats(true); const r = await siakaduService.getKHSStats(); if (r.success) setKhsStats(r.data); } catch (e) { console.error(e); } finally { setIsLoadingKhsStats(false); }
  }, []);
  ...
  useEffect(() => { fetchKrsStats(); fetchKhsStats(); }, [fetchKrsStats, fetchKhsStats]);
  useEffect(() => { fetchKrsData(); }, [fetchKrsData]);
  useEffect(() => { fetchKhsData(); }, [fetchKhsData]);
  ...
  const handleConfirmSync = async () => {
    ...
    try {
      ...
      const syncFn = activeTab === "krs" ? siakaduService.syncKRS : siakaduService.syncKHS;
      const filter = activeTab === "krs" && krsSemester ? { id_semester: krsSemester } : activeTab === "khs" && khsSemester ? { id_semester: khsSemester } : undefined;
      ...
      setTimeout(async () => {
        if (activeTab === "krs") { await fetchKrsStats(); await fetchKrsData(); } else { await fetchKhsStats(); await fetchKhsData(); }
      ...
      render: (item) => (
        <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
          item.status_krs === "Disetujui" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400"
            : item.status_krs === "Batal" ? "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400"
            : "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400"
        }`}>{item.status_krs || "-"}</span>
      ),
    },
  ];
    ...
    { key: "nilai_huruf", label: "Nilai Huruf", align: "center",
      render: (item) => (
        <span className={`px-2 py-1 rounded-full text-xs font-bold ${
          item.nilai_huruf === "A" ? "bg-green-100 text-green-700" : item.nilai_huruf === "B" ? "bg-blue-100 text-blue-700"
            : item.nilai_huruf === "C" ? "bg-amber-100 text-amber-700" : item.nilai_huruf === "D" ? "bg-orange-100 text-orange-700"
            : item.nilai_huruf === "E" ? "bg-red-100 text-red-700" : "bg-gray-100 text-gray-700"
  ...
  const isLoadingCurrentStats = activeTab === "krs" ? isLoadingKrsStats : isLoadingKhsStats;

  return (
    <DashboardLayoutWithDynamicMenu appName="MyUnila Integrator" appIcon={<MdSchool className="w-6 h-6 text-white" />}
      appKey={APP_KEY} fallbackMenus={myunilaIntegratorMenuConfig} pageTitle="KRS/KHS SIAKADU">
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Data KRS & KHS</h1>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Sinkronisasi data KRS dan KHS dari SIAKADU</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card className="bg-gradient-to-br from-blue-500 via-blue-600 to-indigo-600 border-none shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-[1.02] group overflow-hidden relative rounded-xl">
            ...
            <CardBody className="p-5 relative z-10">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-xl bg-white/20 backdrop-blur-sm flex items-center justify-center shadow-lg flex-shrink-0"><FiFileText className="w-7 h-7 text-white" /></div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-blue-100 mb-1">Total KRS</p>
                  {isLoadingKrsStats ? <Spinner size="sm" color="white" /> : <h3 className="text-3xl font-bold text-white tracking-tight leading-none">{(krsStats?.total_records ?? 0).toLocaleString("id-ID")}</h3>}
                </div>
              </div>
            </CardBody>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-emerald-500 via-green-600 to-teal-600 border-none shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-[1.02] group overflow-hidden relative rounded-xl">
            ...
            <CardBody className="p-5 relative z-10">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-xl bg-white/20 backdrop-blur-sm flex items-center justify-center shadow-lg flex-shrink-0"><FiFileText className="w-7 h-7 text-white" /></div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-emerald-100 mb-1">Total KHS</p>
                  {isLoadingKhsStats ? <Spinner size="sm" color="white" /> : <h3 className="text-3xl font-bold text-white tracking-tight leading-none">{(khsStats?.total_records ?? 0).toLocaleString("id-ID")}</h3>}
                </div>
              </div>
            </CardBody>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-violet-500 via-violet-600 to-purple-600 border-none shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-[1.02] group overflow-hidden relative rounded-xl">
            ...
            <CardBody className="p-5 relative z-10">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-xl bg-white/20 backdrop-blur-sm flex items-center justify-center shadow-lg flex-shrink-0"><FiCheckCircle className="w-7 h-7 text-white" /></div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-violet-100 mb-1">Status Sync</p>
                  <h3 className="text-lg font-bold text-white leading-tight">{currentStats?.total_records ? "Tersinkronisasi" : "Belum Sync"}</h3>
                </div>
              </div>
            </CardBody>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-amber-500 via-orange-500 to-yellow-600 border-none shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-[1.02] group overflow-hidden relative rounded-xl">
            ...
            <CardBody className="p-5 relative z-10">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-xl bg-white/20 backdrop-blur-sm flex items-center justify-center shadow-lg flex-shrink-0"><FiClock className="w-7 h-7 text-white" /></div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-amber-100 mb-1">Last Sync</p>
                  {isLoadingCurrentStats ? <Spinner size="sm" color="white" /> : <h3 className="text-base font-bold text-white leading-tight truncate">{formatDate(currentStats?.last_sync)}</h3>}
                </div>
              </div>
            </CardBody>
          </Card>
        ...
        <Card className="border-none shadow-lg rounded-xl overflow-hidden">
          <CardBody className="p-4">
            <Tabs
              ...
              variant="underlined"
              classNames={{ tabList: "gap-6", cursor: "w-full bg-blue-600", tab: "max-w-fit px-0 h-12", tabContent: "group-data-[selected=true]:text-blue-600 font-semibold" }}
            >
              <Tab key="krs" title="📋 KRS (Kartu Rencana Studi)">
                <div className="mt-4">
                  <DataTable<SiakaduKRS> data={krsData} columns={krsColumns} searchable searchPlaceholder="Cari NIM atau nama..."
                    loading={isLoadingKrs} serverSide totalRecords={krsTotalRecords} currentPage={krsPage}
                    onPageChange={setKrsPage} onRowsPerPageChange={(r) => { setKrsRowsPerPage(r); setKrsPage(1); }}
                    onSearchChange={(q) => { setKrsSearch(q); setKrsPage(1); }} defaultRowsPerPage={krsRowsPerPage}
                    filterSlot={
                      <Input size="sm" placeholder="Filter semester (cth: 20241)" value={krsSemester}
                        onValueChange={(val) => { setKrsSemester(val); setKrsPage(1); }}
                        startContent={<FiFilter className="w-4 h-4 text-gray-400" />} className="w-56" classNames={{ inputWrapper: "rounded-xl" }} />
                    } />
                </div>
              </Tab>
              <Tab key="khs" title="📊 KHS (Kartu Hasil Studi)">
                <div className="mt-4">
                  <DataTable<SiakaduKHS> data={khsData} columns={khsColumns} searchable searchPlaceholder="Cari NIM atau nama..."
                    loading={isLoadingKhs} serverSide totalRecords={khsTotalRecords} currentPage={khsPage}
                    onPageChange={setKhsPage} onRowsPerPageChange={(r) => { setKhsRowsPerPage(r); setKhsPage(1); }}
                    onSearchChange={(q) => { setKhsSearch(q); setKhsPage(1); }} defaultRowsPerPage={khsRowsPerPage}
                    filterSlot={
                      <Input size="sm" placeholder="Filter semester (cth: 20241)" value={khsSemester}
                        onValueChange={(val) => { setKhsSemester(val); setKhsPage(1); }}
                        startContent={<FiFilter className="w-4 h-4 text-gray-400" />} className="w-56" classNames={{ inputWrapper: "rounded-xl" }} />
                    } />
                </div>
              </Tab>
        ...
        <ModalContent>{(onClose) => (<>
          <ModalHeader className="flex flex-col gap-1 border-b border-gray-200 dark:border-gray-700">
            <div className="flex items-center gap-3">
              ...
              <div><h3 className="text-xl font-bold text-gray-800 dark:text-white">Konfirmasi Sinkronisasi</h3><p className="text-sm text-gray-500 dark:text-gray-400 font-normal">Data {activeTab.toUpperCase()} SIAKADU</p></div>
            </div>
          </ModalHeader>
          <ModalBody className="py-6">
            <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm text-gray-700 dark:text-gray-300 mb-2">Proses ini akan mengambil data {activeTab.toUpperCase()} terbaru dari SIAKADU.</p>
