...
import siakaduService, { SiakaduTranskrip } from "@/lib/services/siakadu/siakaduService";
...
export default function SiakaduTranskripPage() {
  ...
  const fetchStats = useCallback(async () => {
    try { setIsLoadingStats(true); const r = await siakaduService.getTranskripStats(); if (r.success) setStats(r.data); } catch (e) { console.error(e); } finally { setIsLoadingStats(false); }
  }, []);
  ...
  const handleConfirmSync = async () => {
    ...
    try {
      ...
      const r = await siakaduService.syncTranskrip(nimFilter ? { npm: nimFilter } : undefined, user?.name || "system");
      clearInterval(pi);
      if (r.success) {
  ...
  const filterSlot = (
    <Input size="sm" placeholder="Filter NIM" value={nimFilter}
      onValueChange={(val) => { setNimFilter(val); setCurrentPage(1); }}
      startContent={<FiFilter className="w-4 h-4 text-gray-400" />} className="w-48" classNames={{ inputWrapper: "rounded-xl" }} />
  );

  return (
    <DashboardLayoutWithDynamicMenu appName="MyUnila Integrator" appIcon={<MdSchool className="w-6 h-6 text-white" />}
      appKey={APP_KEY} fallbackMenus={myunilaIntegratorMenuConfig} pageTitle="Transkrip SIAKADU">
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Data Transkrip</h1>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Sinkronisasi data transkrip nilai dari SIAKADU</p>
          </div>
            ...
            <CardBody className="p-5 relative z-10">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-blue-100 mb-1">Total Transkrip</p>
        ...
        <Card className="border-none shadow-lg rounded-xl overflow-hidden">
          <CardBody className="p-0">
            <DataTable<SiakaduTranskrip> data={data} columns={columns} searchable searchPlaceholder="Cari NIM atau nama..."
        ...
        <ModalContent>{(onClose) => (<>
          <ModalHeader className="flex flex-col gap-1 border-b border-gray-200 dark:border-gray-700">
            <div className="flex items-center gap-3">
              ...
              <div><h3 className="text-xl font-bold text-gray-800 dark:text-white">Konfirmasi Sinkronisasi</h3><p className="text-sm text-gray-500 dark:text-gray-400 font-normal">Data Transkrip SIAKADU</p></div>
            </div>
          </ModalHeader>
          <ModalBody className="py-6">
            <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm text-gray-700 dark:text-gray-300 mb-2">Proses ini akan mengambil data transkrip terbaru dari SIAKADU.</p>
                  <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-1 list-disc list-inside">
                    <li>Data yang sudah ada akan diperbarui</li><li>Data baru akan ditambahkan</li>
                  </ul>
                </div>
              </div>
            </div>
