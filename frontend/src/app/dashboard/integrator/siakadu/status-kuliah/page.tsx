...
import siakaduService, { SiakaduStatusKuliah } from "@/lib/services/siakadu/siakaduService";
...
import { FiActivity, FiRefreshCw, FiCheckCircle, FiXCircle, FiClock, FiAlertCircle, FiFilter } from "react-icons/fi";
...
export default function SiakaduStatusKuliahPage() {
  ...
  const fetchStats = useCallback(async () => {
    try { setIsLoadingStats(true); const r = await siakaduService.getStatusKuliahStats(); if (r.success) setStats(r.data); } catch (e) { console.error(e); } finally { setIsLoadingStats(false); }
  }, []);
  ...
  const handleConfirmSync = async () => {
    ...
    try {
      ...
      const r = await siakaduService.syncStatusKuliah(semesterFilter ? { id_semester: semesterFilter } : undefined, user?.name || "system");
      clearInterval(pi);
      if (r.success) {
      ...
      render: (item) => (
        <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
          item.status === "Aktif" || item.status === "A" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400"
            : item.status === "Cuti" || item.status === "C" ? "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400"
            : item.status === "DO" || item.status === "D" ? "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400"
  ...
  const filterSlot = (
    <Input size="sm" placeholder="Filter semester (cth: 20241)" value={semesterFilter}
  ...
  return (
    <DashboardLayoutWithDynamicMenu appName="MyUnila Integrator" appIcon={<MdSchool className="w-6 h-6 text-white" />}
      appKey={APP_KEY} fallbackMenus={myunilaIntegratorMenuConfig} pageTitle="Status Kuliah SIAKADU">
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Status Kuliah</h1>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Sinkronisasi data status kuliah per semester dari SIAKADU</p>
          </div>
            ...
            <CardBody className="p-5 relative z-10">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-blue-100 mb-1">Total Records</p>
        ...
        <Card className="border-none shadow-lg rounded-xl overflow-hidden">
          <CardBody className="p-0">
            <DataTable<SiakaduStatusKuliah> data={data} columns={columns} searchable searchPlaceholder="Cari NIM..."
        ...
        <ModalContent>{(onClose) => (<>
          <ModalHeader className="flex flex-col gap-1 border-b border-gray-200 dark:border-gray-700">
            <div className="flex items-center gap-3">
              ...
              <div><h3 className="text-xl font-bold text-gray-800 dark:text-white">Konfirmasi Sinkronisasi</h3><p className="text-sm text-gray-500 dark:text-gray-400 font-normal">Status Kuliah SIAKADU</p></div>
            </div>
          </ModalHeader>
          <ModalBody className="py-6">
            <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm text-gray-700 dark:text-gray-300 mb-2">Proses ini akan mengambil data status kuliah terbaru dari SIAKADU.</p>
