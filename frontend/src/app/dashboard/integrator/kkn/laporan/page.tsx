...
const SYNC_GROUP = "laporan";

export default function KKNLaporanPage() {
  useRequireAuth();
  const { user } = useAuth();
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Data laporan dan program kerja KKN
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl shadow-lg p-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-xs font-medium text-blue-100">Laporan Kelompok</p>
                {isLoadingStats ? (
                  <div className="h-8 w-20 bg-white/20 rounded animate-pulse" />
                ) : (
                  <h3 className="text-2xl font-bold text-white">{formatNumber(getCount("laporan_kelompok"))}</h3>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-xl shadow-lg p-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-xs font-medium text-emerald-100">Program Kerja</p>
                {isLoadingStats ? (
                  <div className="h-8 w-20 bg-white/20 rounded animate-pulse" />
                ) : (
                  <h3 className="text-2xl font-bold text-white">{formatNumber(getCount("program_kerja"))}</h3>
                )}
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                {isLoadingStats ? (
                  ...
                  <h3 className="text-2xl font-bold text-white">{formatNumber(getCount("laporan_kelompok") + getCount("program_kerja"))}</h3>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full overflow-hidden">
            <div className="p-6">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white">Sinkronisasi Laporan KKN</h3>
                  <p className="text-sm text-gray-500 dark:text-gray-400">Sync data laporan & program kerja</p>
                </div>
              </div>
              <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl mb-6">
                ...
                <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-1 list-disc list-inside">
                  <li>Fetch data laporan kelompok dari e-KKN API</li>
                  <li>Fetch data program kerja mahasiswa</li>
                  <li>Upsert ke SQL Server schema kkn (~110K baris)</li>
                </ul>
                <p className="mt-2 text-xs text-amber-700 dark:text-amber-400">
                  ⏱️ Proses memakan waktu 10-20 menit
                </p>
              </div>
