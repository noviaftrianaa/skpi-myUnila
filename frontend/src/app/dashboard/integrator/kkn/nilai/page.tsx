...
const SYNC_GROUP = "nilai";

export default function KKNNilaiPage() {
  useRequireAuth();
  const { user } = useAuth();
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Data nilai mahasiswa peserta KKN
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl shadow-lg p-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-xs font-medium text-blue-100">Nilai Mahasiswa</p>
                {isLoadingStats ? (
                  <div className="h-8 w-20 bg-white/20 rounded animate-pulse" />
                ) : (
                  <h3 className="text-2xl font-bold text-white">{formatNumber(getCount("nilai_mahasiswa"))}</h3>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-xl shadow-lg p-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-xs font-medium text-emerald-100">Nilai Akhir</p>
                {isLoadingStats ? (
                  <div className="h-8 w-20 bg-white/20 rounded animate-pulse" />
                ) : (
                  <h3 className="text-2xl font-bold text-white">{formatNumber(getCount("nilai_akhir"))}</h3>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-orange-500 to-rose-500 rounded-xl shadow-lg p-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-xs font-medium text-orange-100">Ref Nilai</p>
                {isLoadingStats ? (
                  <div className="h-8 w-20 bg-white/20 rounded animate-pulse" />
                ) : (
                  <h3 className="text-2xl font-bold text-white">{formatNumber(getCount("ref_nilai_kkn"))}</h3>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full overflow-hidden">
            <div className="p-6">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white">Sinkronisasi Nilai KKN</h3>
                  <p className="text-sm text-gray-500 dark:text-gray-400">Sync data nilai dari e-KKN</p>
                </div>
              </div>
              <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl mb-6">
                ...
                <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-1 list-disc list-inside">
                  <li>Fetch data nilai mahasiswa dari e-KKN API</li>
                  <li>Mapping ke anggota kelompok yang sudah tersinkronisasi</li>
                  <li>Upsert ke SQL Server schema kkn (~67K baris)</li>
                </ul>
