...
import { getKategoriCuti, createKategoriCuti, updateKategoriCuti, deleteKategoriCuti } from "@/lib/services/sim-bak/simBakService";
import type { KategoriCuti } from "@/lib/services/sim-bak/types";
import { FiPlus, FiEdit2, FiTrash2, FiX, FiLoader } from "react-icons/fi";
import toast, { Toaster } from "react-hot-toast";

const emptyForm = { id_kategori_cuti: "", nm_kategori: "", deskripsi: "", a_aktif: true, urutan: 1 };

const inputWrap: React.CSSProperties = { borderRadius: "0.5rem", border: "1px solid #d1d5db" };

export default function KategoriCutiTab() {
  const [data, setData] = useState<KategoriCuti[]>([]);
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const result = await getKategoriCuti({ page, limit: 10 });
      setData(result.data);
  ...
  const openEdit = (item: KategoriCuti) => {
    setEditId(item.id_kategori_cuti);
    setForm({ id_kategori_cuti: item.id_kategori_cuti, nm_kategori: item.nm_kategori, deskripsi: item.deskripsi || "", a_aktif: item.a_aktif, urutan: item.urutan });
    setShowPanel(true);
  };

  const handleSave = async () => {
    if (!form.nm_kategori.trim()) { toast.error("Nama kategori wajib diisi"); return; }
    if (!editId && !form.id_kategori_cuti.trim()) { toast.error("ID kategori wajib diisi"); return; }
    setSaving(true);
    try {
      if (editId) {
        await updateKategoriCuti(editId, { nm_kategori: form.nm_kategori, deskripsi: form.deskripsi || undefined, a_aktif: form.a_aktif, urutan: form.urutan });
        toast.success("Kategori cuti berhasil diperbarui");
      } else {
        await createKategoriCuti(form);
        toast.success("Kategori cuti berhasil ditambahkan");
      }
  ...
  const executeDelete = async () => {
    try { await deleteKategoriCuti(deleteConfirmId); toast.success("Berhasil dihapus"); setDeleteConfirmId(""); fetchData(); }
    catch { toast.error("Gagal menghapus"); }
  };

  const columns: Column<KategoriCuti>[] = [
    ...
    { key: "id_kategori_cuti", label: "KODE", width: "150px", render: (item) => <span className="font-mono text-xs bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded">{item.id_kategori_cuti}</span> },
    ...
    { key: "aksi", label: "AKSI", width: "100px", render: (item) => (
      <div className="flex items-center gap-1">
        ...
        <button onClick={() => handleDelete(item.id_kategori_cuti)} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-red-500 dark:text-red-400 transition-colors"><FiTrash2 className="w-4 h-4" /></button>
      </div>
    )},
  ];
  ...
  return (
    ...
    <div className="relative">
      ...
      <DataTable data={data} columns={columns} searchable searchPlaceholder="Cari kategori cuti..." searchKeys={["nm_kategori", "id_kategori_cuti"]} defaultRowsPerPage={10}
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-md bg-white dark:bg-gray-800 shadow-2xl h-full overflow-y-auto animate-in slide-in-from-right">
            <div className="sticky top-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4 flex items-center justify-between z-10">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{editId ? "Edit Kategori Cuti" : "Tambah Kategori Cuti"}</h3>
              ...
              {!editId && (
                <div>
                  ...
                  <div style={inputWrap} className="overflow-hidden">
                    <input type="text" value={form.id_kategori_cuti} onChange={(e) => setForm({ ...form, id_kategori_cuti: e.target.value.toLowerCase().replace(/[^a-z_]/g, '') })}
                      className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700 text-sm text-gray-900 dark:text-white focus:outline-none" placeholder="cth: darurat_bencana" />
                  </div>
              ...
              <div>
                ...
                <div style={inputWrap} className="overflow-hidden">
                  <input type="text" value={form.nm_kategori} onChange={(e) => setForm({ ...form, nm_kategori: e.target.value })}
                    className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700 text-sm text-gray-900 dark:text-white focus:outline-none" placeholder="cth: Cuti Tidak Terencana — Bencana" />
                </div>
              </div>
              <div>
                ...
                <div style={inputWrap} className="overflow-hidden">
                  <textarea value={form.deskripsi} onChange={(e) => setForm({ ...form, deskripsi: e.target.value })} rows={3}
                    className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700 text-sm text-gray-900 dark:text-white focus:outline-none resize-none" placeholder="Penjelasan singkat kategori cuti..." />
                </div>
              </div>
              <div>
    ...
    <ConfirmDialog open={!!deleteConfirmId} title="Hapus Kategori Cuti" message="Hapus kategori cuti ini? Data pengajuan yang sudah menggunakan kategori ini mungkin terpengaruh." confirmLabel="Hapus" confirmColor="danger" onConfirm={executeDelete} onCancel={() => setDeleteConfirmId("")} />
    </>
  );
}
