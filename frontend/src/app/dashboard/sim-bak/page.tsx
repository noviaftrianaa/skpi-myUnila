...
import { FiFileText, FiClock, FiCheckCircle, FiXCircle, FiTrendingUp, FiClipboard, FiUsers, FiArrowRight } from "react-icons/fi";
...
export default function SimBakDashboardPage() {
  ...
  const statCards = [
    { label: "Total Pengajuan", value: stats.total_pengajuan, icon: <FiFileText className="w-6 h-6" />, gradient: "from-blue-500 to-blue-600" },
    { label: "Baru", value: stats.pengajuan_baru, icon: <FiClock className="w-6 h-6" />, gradient: "from-amber-500 to-orange-500" },
    { label: "Dalam Proses", value: stats.pengajuan_proses, icon: <FiTrendingUp className="w-6 h-6" />, gradient: "from-cyan-500 to-teal-500" },
    { label: "Selesai", value: stats.pengajuan_selesai, icon: <FiCheckCircle className="w-6 h-6" />, gradient: "from-emerald-500 to-green-600" },
    { label: "Ditolak", value: stats.pengajuan_ditolak, icon: <FiXCircle className="w-6 h-6" />, gradient: "from-rose-500 to-red-600" },
  ];
                ...
                <div className="relative w-32 h-32">
                  <svg className="w-full h-full transform -rotate-90" viewBox="0 0 120 120">
                    ...
                    <circle cx="60" cy="60" r="52" fill="none" stroke="url(#slaGradient)" strokeWidth="10" strokeLinecap="round"
                      strokeDasharray={`${(stats.sla_compliance / 100) * 327} 327`} />
                    <defs>
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="text-2xl font-bold text-gray-900 dark:text-white">{stats.sla_compliance}%</span>
                    <span className="text-xs text-gray-500 dark:text-gray-400">Tepat Waktu</span>
                  </div>
                </div>
              </div>
              <p className="text-xs text-gray-500 dark:text-gray-400 text-center mt-3">
                Target: Surat Mandiri 5 hari, Permohonan 14 hari
              </p>
            </CardBody>
          </Card>
                ...
                {dummyTrendData.map((item) => {
                  ...
                  return (
                    <div key={item.bulan} className="flex-1 flex flex-col items-center gap-1">
                      ...
                      <div className="w-full flex flex-col-reverse gap-0.5" style={{ height: `${(total / maxVal) * 100}%` }}>
                        <div className="bg-blue-500 rounded-t-sm" style={{ height: `${(item.surat_mandiri / total) * 100}%` }} />
                        <div className="bg-violet-500 rounded-sm" style={{ height: `${(item.permohonan / total) * 100}%` }} />
                        {item.batch > 0 && <div className="bg-amber-500 rounded-sm" style={{ height: `${(item.batch / total) * 100}%` }} />}
                      </div>
                      <span className="text-[10px] text-gray-500 dark:text-gray-400 whitespace-nowrap">{item.bulan.split(" ")[0].slice(0, 3)}</span>
                    </div>
                  );
                })}
              </div>
