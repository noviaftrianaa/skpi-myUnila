...
/**
 * Autocomplete dosen — search by NIDN/NUPTK/NIP/nama.
 * Hit /api/lookup/dosen/search → backend pakai pdrd.sdm WHERE nidn IS NOT NULL.
 */
export function DosenAutocomplete({
  onSelect,
  placeholder = "Cari NIDN, NUPTK, NIP, atau nama dosen...",
}: {
  onSelect: (d: DosenLookup) => void;
  placeholder?: string;
}) {
  const [q, setQ] = useState("");
  const [results, setResults] = useState<DosenLookup[]>([]);
  ...
  useEffect(() => {
    ...
    const h = setTimeout(async () => {
      setLoading(true);
      try {
        const r = await lookupService.searchDosen(q, 10);
        setResults(r);
        setOpen(true);
      } finally {
        setLoading(false);
      ...
      {open && (results.length > 0 || loading) && (
        <div className="absolute z-30 mt-1 max-h-80 w-full overflow-y-auto rounded-lg border border-slate-200 bg-white shadow-lg dark:border-slate-700 dark:bg-slate-800">
          ...
          {!loading &&
            results.map((d, i) => (
              <button
                key={`${d.nidn ?? d.nuptk ?? d.nip ?? d.nama}-${i}`}
                type="button"
                onClick={() => {
                  onSelect(d);
                  setQ("");
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="truncate font-medium text-slate-800 dark:text-slate-100">{d.nama}</div>
                  <div className="flex flex-wrap gap-2 text-xs text-slate-500">
                    {d.nidn && <span className="font-mono">NIDN {d.nidn}</span>}
                    {d.nuptk && <span className="font-mono">NUPTK {d.nuptk}</span>}
                    {d.nip && <span className="font-mono">NIP {d.nip}</span>}
                  </div>
                </div>
              </button>
            ))}
