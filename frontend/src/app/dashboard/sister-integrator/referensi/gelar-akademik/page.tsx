...
export default function GelarAkademikPage() {
  ...
  // State management
  const [gelarAkademikList, setAgamaList] = useState<GelarAkademikData[]>([]);
  ...
  useEffect(() => {
    fetchGelarAkademikData();
  }, []);

  const fetchGelarAkademikData = async () => {
    try {
      setIsLoading(true);
      const data = await gelarAkademikService.getAll();
      setAgamaList(data);
    } catch (error) {
      console.error("Error fetching agama:", error);
      toast.error("Gagal memuat data gelar akademik");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleConfirmSync = async () => {
    ...
    try {
      ...
      // Call sync API
      const response = await gelarAkademikService.sync(user?.username || "system");
      ...
      toast.success(`Berhasil sinkronisasi ${response.data.total_records} data gelar akademik`);

      // Refresh data after 2 seconds
      setTimeout(async () => {
        await fetchGelarAkademikData();
  ...
  const columns: Column<GelarAkademikData>[] = [
    {
      key: "no",
      label: "NO",
      width: "60px",
  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">
              Data Referensi Gelar Akademik
            </h1>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Sinkronisasi dan kelola data referensi gelar akademik dari SISTER API
            </p>
          </div>
          <Button
            color="primary"
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className={`px-2 py-0.5 rounded-full backdrop-blur-sm ${gelarAkademikList.length > 0 ? 'bg-white/20' : 'bg-white/30'}`}>
                      <span className="text-[10px] font-semibold text-white">
                        {gelarAkademikList.length > 0 ? '✓ Active' : '⚠ Pending'}
                      </span>
                    </div>
                  </div>
                  <h3 className="text-2xl font-bold text-white tracking-tight leading-none mb-1">
                    {gelarAkademikList.length > 0 ? "Synced" : "Not Synced"}
                  </h3>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <h3 className="text-base font-bold text-white leading-tight mb-1 truncate">
                    {gelarAkademikList.length > 0 && gelarAkademikList[0].last_sync
                      ? formatDate(gelarAkademikList[0].last_sync)
                      : "Belum pernah"}
                  </h3>
            ...
            <CardBody className="p-4 relative z-10">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <h3 className="text-base font-bold text-white leading-tight mb-1 truncate">
                    {gelarAkademikList.length > 0 && gelarAkademikList[0].synced_by
                      ? gelarAkademikList[0].synced_by
                      : "System"}
                  </h3>
        ...
        {/* Data Table */}
        {gelarAkademikList.length === 0 && !isLoading ? (
            ...
            <CardBody className="p-12">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-6 max-w-md mx-auto">
                  Klik tombol "Sinkronisasi Data" untuk mengambil data referensi gelar akademik dari SISTER API Kemdikbud
                </p>
                <Button
                  color="primary"
                  size="lg"
          ...
          <DataTable
            data={gelarAkademikList}
            columns={columns}
            searchable={true}
            searchKeys={["nama_gelar", "id_gelar_akademik"]}
            searchPlaceholder="Cari gelar akademik..."
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-sm text-gray-700 dark:text-gray-300 mb-2">
                          Proses ini akan mengambil data gelar akademik terbaru dari SISTER API dan menyimpannya ke database.
                        </p>
