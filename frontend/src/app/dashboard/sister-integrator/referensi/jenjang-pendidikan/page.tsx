...
import { jenjangPendidikanService, type JenjangPendidikanData } from "@/lib/services/sister/referensi/jenjangPendidikanService";
...
export default function JenjangPendidikanPage() {
  ...
  // State management
  const [jenjangPendidikanList, setAgamaList] = useState<JenjangPendidikanData[]>([]);
  ...
  useEffect(() => {
    fetchJenjangPendidikanData();
  }, []);

  const fetchJenjangPendidikanData = async () => {
    try {
      setIsLoading(true);
      const data = await jenjangPendidikanService.getAll();
      setAgamaList(data);
    } catch (error) {
  ...
  const handleConfirmSync = async () => {
    ...
    try {
      ...
      // Call sync API
      const response = await jenjangPendidikanService.sync(user?.username || "system");
      ...
      toast.success(`Berhasil sinkronisasi ${response.data.total_records} data jenjang pendidikan`);

      // Refresh data after 2 seconds
      setTimeout(async () => {
        await fetchJenjangPendidikanData();
  ...
  const columns: Column<JenjangPendidikanData>[] = [
    {
      key: "no",
      label: "NO",
      width: "60px",
  ...
  return (
    <DashboardLayout
      ...
      pageTitle="Referensi - Jenjang Pendidikan"
    >
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Sinkronisasi dan kelola data referensi jenjang pendidikan dari SISTER API
            </p>
          </div>
          <Button
            color="primary"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card className="bg-gradient-to-br from-emerald-500 via-green-600 to-teal-600 border-none shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-[1.02] group overflow-hidden relative">
            ...
            <CardBody className="p-4 relative z-10">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-xl bg-white/20 backdrop-blur-sm flex items-center justify-center shadow-lg group-hover:rotate-6 transition-transform duration-300 flex-shrink-0">
                  {jenjangPendidikanList.length > 0 ? (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className={`px-2 py-0.5 rounded-full backdrop-blur-sm ${jenjangPendidikanList.length > 0 ? 'bg-white/20' : 'bg-white/30'}`}>
                      <span className="text-[10px] font-semibold text-white">
                        {jenjangPendidikanList.length > 0 ? '✓ Active' : '⚠ Pending'}
                      </span>
                    </div>
                  </div>
                  <h3 className="text-2xl font-bold text-white tracking-tight leading-none mb-1">
                    {jenjangPendidikanList.length > 0 ? "Synced" : "Not Synced"}
                  </h3>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <h3 className="text-base font-bold text-white leading-tight mb-1 truncate">
                    {jenjangPendidikanList.length > 0 && jenjangPendidikanList[0].last_sync
                      ? formatDate(jenjangPendidikanList[0].last_sync)
                      : "Belum pernah"}
                  </h3>
            ...
            <CardBody className="p-4 relative z-10">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <h3 className="text-base font-bold text-white leading-tight mb-1 truncate">
                    {jenjangPendidikanList.length > 0 && jenjangPendidikanList[0].synced_by
                      ? jenjangPendidikanList[0].synced_by
                      : "System"}
                  </h3>
        ...
        {/* Data Table */}
        {jenjangPendidikanList.length === 0 && !isLoading ? (
            ...
            <CardBody className="p-12">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-6 max-w-md mx-auto">
                  Klik tombol "Sinkronisasi Data" untuk mengambil data referensi jenjang pendidikan dari SISTER API Kemdikbud
                </p>
                <Button
                  color="primary"
                  size="lg"
          ...
          <DataTable
            data={jenjangPendidikanList}
            columns={columns}
            searchable={true}
            searchKeys={["nama_jenjang", "id_jenjang_pendidikan"]}
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-sm text-gray-700 dark:text-gray-300 mb-2">
                          Proses ini akan mengambil data jenjang pendidikan terbaru dari SISTER API dan menyimpannya ke database.
                        </p>
