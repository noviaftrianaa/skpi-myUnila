...
export default function JabatanFungsionalPage() {
  ...
  // State
  const [stats, setStats] = useState<RiwayatFungsionalStats | null>(null);
  ...
  const fetchStats = async () => {
    ...
    try {
      const data = await sisterRiwayatFungsionalService.getStats();
      setStats(data);
    } catch (error) {
      console.error("Error loading stats:", error);
      toast.error("Gagal memuat statistik jabatan fungsional");
    } finally {
  ...
  const handleConfirmSync = async () => {
    ...
    try {
      ...
      const result = await sisterRiwayatFungsionalService.syncFromSister(username);
      ...
      setSyncResult({
        ...
        message: `Berhasil sync ${result.total_success} dosen dengan jabatan fungsional`,
      });
    ...
    >
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Kelola dan sinkronisasi data jabatan fungsional dosen dari SISTER API
            </p>
          </div>
          <Button
            color="primary"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-xs font-medium text-blue-100">Total Jabatan</p>
                  ...
                  <h3 className="text-3xl font-bold text-white tracking-tight leading-none">
                    {isLoadingStats ? "..." : stats?.total_riwayat || 0}
                  </h3>
                  <p className="text-[10px] text-blue-100/80 mt-1">
                    Records jabatan fungsional
                  </p>
                </div>
              </div>
            </CardBody>
          ...
          {/* Profesor Card */}
          <Card className="bg-gradient-to-br from-red-500 via-red-600 to-rose-600 border-none shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-[1.02] group overflow-hidden relative">
            <div className="absolute top-0 right-0 w-24 h-24 bg-white/10 rounded-full -mr-12 -mt-12 group-hover:scale-150 transition-transform duration-500" />
            <div className="absolute bottom-0 left-0 w-20 h-20 bg-white/5 rounded-full -ml-10 -mb-10 group-hover:scale-125 transition-transform duration-700" />
            <CardBody className="p-4 relative z-10">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-xs font-medium text-red-100">Profesor</p>
                    <div className="px-2 py-0.5 rounded-full bg-white/20 backdrop-blur-sm">
                      <span className="text-[10px] font-semibold text-white">PROF</span>
                    </div>
                  </div>
                  <h3 className="text-3xl font-bold text-white tracking-tight leading-none">
                    {isLoadingStats ? "..." : stats?.total_profesor || 0}
                  </h3>
                  <p className="text-[10px] text-red-100/80 mt-1">
                    Guru Besar
                  </p>
                </div>
              </div>
          ...
          <Card className="bg-gradient-to-br from-orange-500 via-amber-600 to-yellow-600 border-none shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-[1.02] group overflow-hidden relative">
            ...
            <CardBody className="p-4 relative z-10">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-xs font-medium text-orange-100">Lektor Kepala</p>
                    <div className="px-2 py-0.5 rounded-full bg-white/20 backdrop-blur-sm">
                      <span className="text-[10px] font-semibold text-white">LK</span>
                    </div>
                  </div>
                  <h3 className="text-3xl font-bold text-white tracking-tight leading-none">
                    {isLoadingStats ? "..." : stats?.total_lektor_kepala || 0}
                  </h3>
        ...
        <ScheduleList syncType="jabatan_fungsional" />

        {/* Data Table */}
        <SisterRiwayatFungsionalTable />
      </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm text-gray-700 dark:text-gray-300 mb-2">
                      Sistem akan melakukan sinkronisasi semua data jabatan fungsional dosen dari SISTER API.
                    </p>
              ...
              {syncStatus === "syncing" && (
                <div className="space-y-3">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 text-center">
                    Sedang memproses data jabatan fungsional dosen...
                  </p>
                </div>
              )}
