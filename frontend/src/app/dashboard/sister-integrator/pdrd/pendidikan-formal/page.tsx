...
export default function PendidikanFormalPage() {
  ...
  // State
  const [stats, setStats] = useState<PendidikanFormalStats | null>(null);
  ...
  const fetchStats = async () => {
    try {
      ...
      const data = await sisterPendidikanService.getStats();
      setStats(data);
    } catch (error) {
      console.error("Error fetching stats:", error);
      toast.error("Gagal memuat statistik pendidikan formal");
    } finally {
  ...
  const handleConfirmSync = async () => {
    ...
    try {
      ...
      const result = await sisterPendidikanService.syncFromSister(username);
      ...
      setSyncResult({
        totalRecords: result.total_pendidikan_formal,
        message: `Berhasil sync ${result.total_success} dosen dengan total ${result.total_pendidikan_formal} data pendidikan formal`,
      });
  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Kelola dan sinkronisasi data riwayat pendidikan formal dosen dari SISTER API
            </p>
          </div>
          <Button
            color="primary"
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-xs font-medium text-blue-100">Jenjang S1</p>
                    <div className="px-2 py-0.5 rounded-full bg-white/20 backdrop-blur-sm">
                      <span className="text-[10px] font-semibold text-white">🎓 S1</span>
                    </div>
                  </div>
                  <h3 className="text-3xl font-bold text-white tracking-tight leading-none mb-1">
                    {stats?.total_s1.toLocaleString() || "0"}
                  </h3>
          ...
          <Card className="bg-gradient-to-br from-emerald-500 via-green-600 to-teal-600 border-none shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-[1.02] group overflow-hidden relative">
            ...
            <CardBody className="p-4 relative z-10">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-xs font-medium text-emerald-100">Jenjang S2</p>
                    <div className="px-2 py-0.5 rounded-full bg-white/20 backdrop-blur-sm">
                      <span className="text-[10px] font-semibold text-white">🎓 S2</span>
                    </div>
                  </div>
                  <h3 className="text-3xl font-bold text-white tracking-tight leading-none mb-1">
                    {stats?.total_s2.toLocaleString() || "0"}
                  </h3>
          ...
          <Card className="bg-gradient-to-br from-orange-500 via-amber-600 to-yellow-600 border-none shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-[1.02] group overflow-hidden relative">
            ...
            <CardBody className="p-4 relative z-10">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-xs font-medium text-orange-100">Jenjang S3</p>
                    <div className="px-2 py-0.5 rounded-full bg-white/30 backdrop-blur-sm">
                      <span className="text-[10px] font-semibold text-white">🎓 S3</span>
                    </div>
                  </div>
                  <h3 className="text-3xl font-bold text-white tracking-tight leading-none mb-1">
                    {stats?.total_s3.toLocaleString() || "0"}
                  </h3>
        ...
        <ScheduleList syncType="pendidikan" />

        {/* Data Table */}
        <SisterPendidikanTable />
      </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm text-gray-700 dark:text-gray-300 mb-2">
                      Sistem akan melakukan sinkronisasi semua data pendidikan formal dosen dari SISTER API.
                    </p>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 font-normal">
                  ...
                  {syncStatus === "success" && syncResult && `${syncResult.totalRecords} pendidikan formal berhasil disinkronkan`}
              ...
              {syncStatus === "syncing" && (
                <div className="space-y-3">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 text-center">
                    Sedang memproses data pendidikan formal dosen...
                  </p>
                </div>
              )}
