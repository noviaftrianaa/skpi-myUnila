...
export default function BidangIlmuPage() {
  ...
  // State
  const [stats, setStats] = useState<BidangIlmuStats | null>(null);
  ...
  const fetchStats = async () => {
    ...
    try {
      const data = await sisterBidangIlmuService.getStats();
      setStats(data);
    } catch (error) {
      console.error("Error loading stats:", error);
      toast.error("Gagal memuat statistik bidang ilmu");
    } finally {
  ...
  const handleConfirmSync = async () => {
    ...
    try {
      ...
      const result = await sisterBidangIlmuService.syncFromSister(username);
      ...
      setSyncResult({
        ...
        message: `Berhasil sync ${result.total_success} dosen dengan bidang ilmu`,
      });
  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Kelola dan sinkronisasi data bidang ilmu/keahlian dosen dari SISTER API
            </p>
          </div>
          <Button
            color="primary"
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-xs font-medium text-blue-100">Total Dosen</p>
                  ...
                  <h3 className="text-3xl font-bold text-white tracking-tight leading-none">
                    {isLoadingStats ? "..." : stats?.total_dosen || 0}
                  </h3>
          ...
          <Card className="bg-gradient-to-br from-green-500 via-green-600 to-emerald-600 border-none shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-[1.02] group overflow-hidden relative">
            ...
            <CardBody className="p-4 relative z-10">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-xs font-medium text-green-100">Total Bidang Ilmu</p>
                    <div className="px-2 py-0.5 rounded-full bg-white/20 backdrop-blur-sm">
                      <span className="text-[10px] font-semibold text-white">Maps</span>
                    </div>
                  </div>
                  <h3 className="text-3xl font-bold text-white tracking-tight leading-none">
                    {isLoadingStats ? "..." : stats?.total_bidang_ilmu || 0}
                  </h3>
                  <p className="text-[10px] text-green-100/80 mt-1">
                    Mapping bidang ilmu dosen
                  </p>
                </div>
              </div>
            </CardBody>
        ...
        <ScheduleList syncType="bidang_ilmu" />

        {/* Data Table */}
        <SisterBidangIlmuTable />
      </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm text-gray-700 dark:text-gray-300 mb-2">
                      Sistem akan melakukan sinkronisasi semua data bidang ilmu dosen dari SISTER API.
                    </p>
              ...
              {syncStatus === "syncing" && (
                <div className="space-y-3">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 text-center">
                    Sedang memproses data bidang ilmu dosen...
                  </p>
                </div>
              )}
