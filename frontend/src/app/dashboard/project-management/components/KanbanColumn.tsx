...
interface KanbanColumnProps {
  ...
  title: string;
  tasks: Task[];
  /** @deprecated colorClass is no longer used — color is derived from id */
  colorClass?: string;
...
const STATUS_STYLE: Record<
  string,
  {
    header: string;        // header bg (light / dark)
    accent: string;        // left border accent colour
    dot: string;           // coloured circle in header
    badge: string;         // count badge colours
    column: string;        // column background
  }
> = {
  backlog: {
  ...
  return (
    <div
      className={[
        ...
        "w-full lg:min-w-[280px] lg:max-w-[320px] lg:w-[300px]",
        style.column,
        style.accent,
        "shadow-sm",
        ...
        {(provided, snapshot) => (
          <div
            ...
            className={[
              ...
              "min-h-[200px]",
              "max-h-[calc(100vh-300px)] lg:max-h-[calc(100vh-260px)]",
              "overflow-y-auto",
              ...
              snapshot.isDraggingOver
                ? "bg-white/70 dark:bg-white/5 ring-2 ring-inset ring-[#0B5EA8]/30 rounded-lg border-2 border-dashed border-[#0B5EA8]/40"
                : "",
            ].join(" ")}
          >
