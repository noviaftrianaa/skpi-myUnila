    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
          <FiActivity className="w-6 h-6 text-[#0B5EA8]" />
          Analytics
        </h1>
        <p className="text-sm text-gray-500 mt-1">Pilih project untuk melihat analytics detail.</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {projects.map((p) => (
          <Link key={p.id} href={`/dashboard/project-management/${p.id}/analytics`}>
