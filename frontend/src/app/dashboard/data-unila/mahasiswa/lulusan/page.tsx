...
export default function LulusanPage() {
  ...
  useEffect(() => {
    mahasiswaDataService.getLulusanStats({}).then(setStats).catch(console.error);
  ...
  const columns: Column<LulusanItem>[] = [
    { key: "nm_pd", label: "NAMA LULUSAN", sortable: true, render: (i) => (
      <div>
        ...
        <div className="text-xs text-gray-500">{i.nipd} · {i.jk === "L" ? "L" : "P"}</div>
      </div>
    )},
    { key: "nm_prodi", label: "PROGRAM STUDI", sortable: true, render: (i) => (
      <div>
        <div className="text-sm text-gray-700 dark:text-gray-300">{i.nm_prodi}</div>
        <div className="text-xs text-gray-500">{i.nm_fakultas} · {i.jenjang}</div>
      </div>
    )},
    { key: "angkatan", label: "ANGKATAN", width: "90px", sortable: true, align: "center" as const, render: (i) => (
    ...
    { key: "ipk", label: "IPK", width: "70px", sortable: true, align: "right" as const, render: (i) => (
      <span className={`font-mono text-sm font-bold ${Number(i.ipk) >= 3.5 ? "text-green-600 dark:text-green-400" : Number(i.ipk) >= 3.0 ? "text-blue-600 dark:text-blue-400" : "text-gray-600"}`}>
    ...
    >
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Alumni Universitas Lampung</p>
        </div>

        {stats && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { l: "Total Lulusan", v: fmtNum(stats.total), c: "from-green-500 to-emerald-600", ic: <FiUsers className="w-6 h-6"/> },
              { l: "Rata-rata IPK", v: Number(stats.avg_ipk || 0).toFixed(2), c: "from-blue-500 to-indigo-600", ic: <FiAward className="w-6 h-6"/>, raw: true },
              { l: "Total Prodi", v: fmtNum(stats.total_prodi), c: "from-violet-500 to-purple-600", ic: <FiGrid className="w-6 h-6"/> },
              { l: "Total Angkatan", v: fmtNum(stats.total_angkatan), c: "from-amber-500 to-orange-500", ic: <FiTrendingUp className="w-6 h-6"/> },
            ].map(s => (
