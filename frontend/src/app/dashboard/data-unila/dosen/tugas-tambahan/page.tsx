...
import { FiBriefcase, FiUsers, FiCheckCircle, FiGrid, FiFilter, FiX } from "react-icons/fi";
import { Toaster } from "react-hot-toast";
import toast from "react-hot-toast";
import { dataUnilaMenuConfig } from "../../config/menuConfig";
import dosenDataService, {
  type TugasTambahanItem, type TugasTambahanStats,
...
export default function TugasTambahanPage() {
  ...
  const [data, setData] = useState<TugasTambahanItem[]>([]);
  ...
  const [sortBy, setSortBy] = useState("tmt_mulai");
  ...
  useEffect(() => {
    ...
    dosenDataService.getTugasTambahanStats({
  ...
  useEffect(() => {
    setLoading(true);
    dosenDataService.getTugasTambahanList({
      page, limit,
  ...
  const jabatanOptions: DropdownOption[] = (stats?.by_jabatan || [])
    .map((j) => ({ value: j.nm_jabatan, label: j.nm_jabatan, sublabel: `${j.jumlah}` }));
  ...
  const handleExport = (fmtType: ExportFormat) => {
    ...
    const baseName = `tugas-tambahan-dosen-${filterJabatan || "all"}`;
    if (fmtType === "excel") {
      exportToExcel(dataForExport as unknown as Record<string, unknown>[], baseName, "TugasTambahan", EXPORT_HEADERS);
      ...
      exportToPdf(dataForExport as unknown as Record<string, unknown>[], baseName, {
        title: "Tugas Tambahan Dosen Universitas Lampung",
  ...
  const isAktif = (i: TugasTambahanItem): boolean => {
    if (!i.tmt_selesai) return true;
    try {
      const tst = new Date(i.tmt_selesai.replace(" ", "T"));
      return tst.getTime() > Date.now();
    } catch { return false; }
  };

  const columns: Column<TugasTambahanItem>[] = [
    {
    ...
    {
      key: "jabatan_tambahan", label: "JABATAN TAMBAHAN", sortable: true,
      render: (i) => (
        <div>
          <div className="text-sm font-medium text-gray-800 dark:text-gray-200 line-clamp-1">{i.jabatan_tambahan || "—"}</div>
          {i.unit_tugas && <div className="text-xs text-gray-500 dark:text-gray-400 line-clamp-1">{i.unit_tugas}</div>}
        </div>
      ),
    },
    {
      key: "no_sk", label: "NO SK", width: "160px",
      render: (i) => (
        <span className="text-xs font-mono text-gray-700 dark:text-gray-300 truncate max-w-[150px] block" title={i.no_sk}>
          {i.no_sk || "—"}
        </span>
      ),
    },
    {
      key: "tmt_mulai", label: "TMT MULAI", width: "120px", sortable: true,
      render: (i) => <span className="text-xs font-mono text-gray-600 dark:text-gray-400">{fmtDate(i.tmt_mulai)}</span>,
    },
    {
      key: "tmt_selesai", label: "TMT SELESAI", width: "120px",
      render: (i) => <span className="text-xs font-mono text-gray-600 dark:text-gray-400">{fmtDate(i.tmt_selesai)}</span>,
    },
    {
      key: "status", label: "STATUS", width: "100px", align: "center" as const,
      render: (i) => {
        const aktif = isAktif(i);
        return (
          <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold ring-1 ring-inset ${aktif ? "bg-emerald-100 text-emerald-700 ring-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-300" : "bg-slate-100 text-slate-600 ring-slate-200 dark:bg-slate-500/10 dark:text-slate-300"}`}>
            {aktif ? "Aktif" : "Selesai"}
          </span>
        );
      },
    ...
    >
      ...
      <div className="space-y-6">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Tugas Tambahan Dosen</h1>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Penugasan tambahan dosen — jabatan, SK, masa berlaku, dan unit tugas</p>
          </div>
          {hasFilter && (
        ...
        {loadingStats ? (
          ...
          return (
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              <StatCard icon={<FiBriefcase className="w-6 h-6" />} label="Total Tugas" value={total} gradient="from-blue-500 to-indigo-600" />
              <StatCard icon={<FiCheckCircle className="w-6 h-6" />} label="Sedang Berjalan" value={aktif} gradient="from-emerald-500 to-teal-600" subtext={`${pct(aktif, total)} dari total`} />
              <StatCard icon={<FiUsers className="w-6 h-6" />} label="Dosen Tertugas" value={totalDosen} gradient="from-violet-500 to-purple-600" />
              <StatCard icon={<FiGrid className="w-6 h-6" />} label="Jenis Jabatan" value={num(stats.total_jabatan)} gradient="from-amber-500 to-orange-500" />
            </div>
          );
        })()}
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-md overflow-hidden border border-gray-200/50 dark:border-gray-800">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <div className="p-4 sm:p-5 space-y-4 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-3">
                ...
                {jabatanOptions.length > 0 && (
                  <Dropdown label="Jabatan Tambahan" value={filterJabatan}
            ...
            <DataTable columns={columns} data={data} loading={loading} serverSide totalRecords={total}
              ...
              searchPlaceholder="Cari nama dosen, NIDN, jabatan, no SK..."
