...
export default function SertifikasiPage() {
  ...
  const [sortBy, setSortBy] = useState("tgl_sert");
  ...
  useEffect(() => {
    dosenDataService.getSertifikasiStats({}).then(setStats).catch(console.error);
  ...
  const columns: Column<SertifikasiItem>[] = [
    { key: "nm_sdm", label: "NAMA DOSEN", sortable: true, render: (i) => (
      <div>
        ...
        <div className="text-xs text-gray-500">{i.nidn || i.nip || "-"} · {i.nm_prodi || "-"}</div>
      </div>
    )},
    { key: "bidang_studi", label: "BIDANG STUDI", render: (i) => (
      <div className="text-sm text-gray-800 dark:text-gray-200 font-medium line-clamp-1">{i.bidang_studi || "-"}</div>
    )},
      ...
      <div className="space-y-6">
        ...
        {stats && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { l: "Total Sertifikasi", v: fmtNum(stats.total), c: "from-blue-500 to-indigo-600", ic: <FiFileText className="w-6 h-6"/> },
              { l: "Total Dosen", v: fmtNum(stats.total_dosen), c: "from-green-500 to-emerald-600", ic: <FiUsers className="w-6 h-6"/> },
              { l: "Jenis Sertifikasi", v: fmtNum(stats.total_jenis), c: "from-violet-500 to-purple-600", ic: <FiGrid className="w-6 h-6"/> },
              { l: "Rentang Tahun", v: fmtNum(stats.total_tahun), c: "from-amber-500 to-orange-500", ic: <FiCalendar className="w-6 h-6"/> },
            ].map(s => (
          ...
          <CardBody className="p-0">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <DataTable
                ...
                searchPlaceholder="Cari nama dosen, NIDN, nama sertifikasi..."
