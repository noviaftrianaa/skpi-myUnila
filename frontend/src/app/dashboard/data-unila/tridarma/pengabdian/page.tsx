...
import { FiUsers, FiDollarSign, FiFilter, FiRotateCcw, FiX, FiAward, FiMapPin } from "react-icons/fi";
...
export default function PengabdianPage() {
  ...
  const handleExport = (fmtType: ExportFormat) => {
    ...
    const baseName = `pengabdian-${filterTahun || "all"}`;
    if (fmtType === "excel") exportToExcel(dataForExport as unknown as Record<string, unknown>[], baseName, "Pengabdian", EXPORT_HEADERS);
    else if (fmtType === "csv-client") exportToCsv(dataForExport as unknown as Record<string, unknown>[], baseName, EXPORT_HEADERS);
    else if (fmtType === "pdf") exportToPdf(dataForExport as unknown as Record<string, unknown>[], baseName, { title: "Pengabdian Masyarakat Universitas Lampung", headers: EXPORT_HEADERS, orientation: "landscape" });
  ...
  const columns: Column<LitabmasItem>[] = [
    ...
    { key: "tahun", label: "TAHUN", width: "90px", sortable: true, align: "center" as const, render: (i) => (
      <span className="inline-flex items-center justify-center px-2.5 py-0.5 rounded-md text-xs font-bold font-mono bg-teal-50 text-teal-700 ring-1 ring-teal-200 dark:bg-teal-500/10 dark:text-teal-300">{i.tahun}</span>
    )},
    { key: "total_dana", label: "TOTAL DANA", width: "150px", sortable: true, align: "right" as const, render: (i) => (
      <span className="font-mono text-xs text-gray-700 dark:text-gray-300">{i.total_dana ? fmtRupiah(num(i.total_dana)) : "—"}</span>
    )},
    { key: "lokasi_kegiatan", label: "LOKASI", width: "200px", render: (i) => (
  ...
  return (
    <DashboardLayoutWithDynamicMenu appName="Data Unila" appIcon={<MdSchool className="w-6 h-6 text-white" />} appKey={APP_KEY} fallbackMenus={dataUnilaMenuConfig} pageTitle="Pengabdian">
      <Toaster position="top-right" />
      <div className="space-y-6">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Pengabdian Masyarakat</h1>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Litabmas pengabdian kepada masyarakat civitas akademika</p>
          </div>
          {filterTahun && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-full bg-teal-50 text-teal-700 ring-1 ring-teal-200 dark:bg-teal-500/10 dark:text-teal-300">
        ...
        {loadingStats ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <StatCard icon={<FiUsers className="w-6 h-6" />} label="Total Litabmas" value={num(stats.total)} gradient="from-blue-500 to-indigo-600" subtext={`+${fmt(num(stats.penelitian))} penelitian`} />
            ...
            <StatCard icon={<FiAward className="w-6 h-6" />} label="Pengabdian %" value={stats.total && num(stats.total) > 0 ? `${((num(stats.pengabdian) / num(stats.total)) * 100).toFixed(1)}%` : "—"} gradient="from-pink-500 to-rose-600" subtext="dari total litabmas" />
          </div>
        )}
            ...
            <div className="p-4 sm:p-5 space-y-4 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800">
              ...
              {filterTahun && (
                <div className="flex flex-wrap items-center gap-1.5 pt-1">
                  ...
                  <span className="inline-flex items-center gap-1 pl-2.5 pr-1.5 py-1 text-xs font-medium bg-teal-50 text-teal-700 ring-1 ring-teal-200 rounded-full dark:bg-teal-500/10 dark:text-teal-300">
                    Tahun {filterTahun}
                    <button type="button" onClick={() => { setFilterTahun(""); setPage(1); }} className="ml-0.5 w-4 h-4 rounded-full hover:bg-teal-200 dark:hover:bg-teal-400/20 flex items-center justify-center"><FiX className="w-3 h-3" /></button>
                  </span>
                </div>
              )}
            </div>
