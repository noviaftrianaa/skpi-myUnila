...
import { MdSync, MdSchool, MdEmojiEvents } from "react-icons/md";
...
interface PrestasiStats {
  ...
  total_nasional: number;
  total_internasional: number;
  last_sync?: string;
}
...
export default function PrestasiManagementPage() {
  ...
  // Handle filter changes from table (for future use)
  const handleFilterChange = useCallback(
    (filters: { tahun_prestasi?: number }) => {
      // Can be used to sync based on current filter
    },
    []
  );
  ...
  const fetchStats = async () => {
    try {
      ...
      const response = await feederClient.get("/prestasi/stats", {
        params: { _t: Date.now() },
      });
      ...
      toast.error("Gagal memuat statistik prestasi");
    } finally {
  ...
  const handleConfirmSync = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setSyncResult({
          ...
          message:
            ...
            "Data prestasi berhasil disinkronkan",
        });
      ...
      console.error("Error syncing prestasi:", error);
      setSyncStatus("error");
    ...
    >
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Data prestasi mahasiswa dari Neo Feeder PDDIKTI
            </p>
          </div>
          <Button
            color="primary"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {/* Total Prestasi Card */}
          <Card className="bg-gradient-to-br from-amber-500 via-amber-600 to-orange-600 border-none shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-[1.02] group overflow-hidden relative">
                  ...
                  {isLoadingStats ? (
                    ...
                    <>
                      <h3 className="text-3xl font-bold text-white tracking-tight leading-none mb-1">
                        {stats?.total_prestasi?.toLocaleString() || "0"}
                      </h3>
                      <p className="text-[10px] text-amber-100/80 flex items-center gap-1">
                        <span className="w-1.5 h-1.5 bg-yellow-400 rounded-full animate-pulse" />
                        Total record prestasi
                      </p>
                    </>
                  )}
                </div>
                  ...
                  {isLoadingStats ? (
                    ...
                    <div className="space-y-0.5">
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-purple-100">Internasional:</span>
                        <span className="font-semibold text-white">{stats?.total_internasional?.toLocaleString() || "0"}</span>
                      </div>
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-purple-100">Nasional:</span>
                        <span className="font-semibold text-white">{stats?.total_nasional?.toLocaleString() || "0"}</span>
                      </div>
                    </div>
                  )}
                </div>
        ...
        <ScheduleList syncType={"prestasi" as any} />

        {/* Data Table */}
        <FeederPrestasiTable onFilterChange={handleFilterChange} />
      </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 font-normal">
                      Data Prestasi Mahasiswa Neo Feeder
                    </p>
                  </div>
                </div>
              </ModalHeader>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="w-full">
                        <p className="text-sm text-gray-700 dark:text-gray-300 mb-2">
                          Proses ini akan mengambil data prestasi mahasiswa terbaru dari Neo Feeder PDDIKTI dan menyimpannya ke database.
                        </p>
