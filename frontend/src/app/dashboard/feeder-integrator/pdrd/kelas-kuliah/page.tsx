...
export default function KelasKuliahManagementPage() {
  useRequireAuth();
  const { user } = useAuth();

  // State
  const [stats, setStats] = useState<KelasKuliahStats | null>(null);
  ...
  const fetchStats = async () => {
    try {
      ...
      const response = await feederClient.get("/kelas-kuliah/stats", {
        params: { _t: Date.now() },
      });
      ...
      toast.error("Gagal memuat statistik kelas kuliah");
    } finally {
  ...
  const handleConfirmSync = async () => {
    ...
    try {
      ...
      const response = await feederClient.post(
        `/kelas-kuliah/sync?${params.toString()}`
      );
      ...
      if (response.data.success) {
        ...
        setSyncResult({
          totalRecords: response.data.data?.total_kelas || 0,
          totalDosen: response.data.data?.total_dosen || 0,
          message:
            response.data.message ||
            "Data kelas kuliah berhasil disinkronkan",
        });
      ...
      console.error("Error syncing kelas kuliah:", error);
      setSyncStatus("error");
  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Data kelas kuliah dan dosen pengajar dari Neo Feeder PDDIKTI
            </p>
          </div>
          <Button
            color="primary"
        ...
        <ScheduleList syncType={"kelas_kuliah" as any} />

        {/* Data Table */}
        <FeederKelasKuliahTable onFilterChange={handleFilterChange} />
      </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="w-full">
                        <p className="text-sm text-gray-700 dark:text-gray-300 mb-2">
                          Proses ini akan mengambil data kelas kuliah dan dosen pengajar
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-600 dark:text-gray-400">
                          ...
                          di-sync bulan ini. Data yang gagal sync akan dicoba
                          lagi.
                        </p>
                      </div>
                      <Switch
                    ...
                    {forceSync && (
                      <div className="mt-2 p-2 rounded bg-yellow-100 dark:bg-yellow-900/30">
                        <p className="text-xs text-yellow-800 dark:text-yellow-200 flex items-center gap-1">
                          ...
                          <span>
                            Mode paksa sync aktif - akan melewati pengecekan
                            "sudah sync bulan ini"
                          </span>
                        </p>
                      </div>
                    )}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 font-normal">
                  ...
                  {syncStatus === "success" &&
                    syncResult &&
                    `${syncResult.totalRecords} kelas, ${syncResult.totalDosen} dosen berhasil disinkronkan`}
                  {syncStatus === "error" &&
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">
                        Total: {syncResult.totalRecords} kelas kuliah, {syncResult.totalDosen} dosen
                      </p>
                    </div>
                  </div>
                </div>
