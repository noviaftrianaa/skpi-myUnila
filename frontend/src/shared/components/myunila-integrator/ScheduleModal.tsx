  ...
  useEffect(() => {
    if (schedule) {
      ...
      const getDefaultName = () => {
        return `${getSyncTypeLabel(syncType)} Sync ${timestamp}`;
      };

      const getDefaultDescription = () => {
        return `Sync ${getSyncTypeLabel(syncType)} data every day`;
      };
        ...
        <form onSubmit={handleSubmit}>
          <ModalHeader className="flex flex-col gap-2 px-6 py-5 border-b border-gray-200 dark:border-slate-700">
            ...
            <p className="text-sm font-normal text-gray-600 dark:text-slate-400">
              Schedule sync untuk {getSyncTypeLabel(syncType)}
            </p>
          </ModalHeader>
          ...
          <ModalBody className="gap-5 py-6">
            ...
            <div className="space-y-2">
              ...
              <Input
                placeholder="Contoh: Daily Unit Organisasi Sync"
                value={formData.name}
                onChange={(e) =>
