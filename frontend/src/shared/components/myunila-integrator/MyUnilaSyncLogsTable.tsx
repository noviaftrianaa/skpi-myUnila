...
import type { MyUnilaSyncLog as SyncLog } from "@/lib/services/myunila/management/syncLogsService";
import { FiCheckCircle, FiXCircle, FiAlertCircle, FiClock, FiEye } from "react-icons/fi";
import { toast } from "react-hot-toast";

export default function MyUnilaSyncLogsTable() {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const response = await myunilaSyncLogsService.getSyncLogs({
          page: currentPage,
          limit: rowsPerPage,
          search: searchQuery || undefined,
          status: filterStatus as any || undefined,
          sync_type: filterSyncType as any || undefined,
        });
                ...
                <div>
                  ...
                  <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3">
                    <p className="text-sm text-red-700 dark:text-red-400 font-mono whitespace-pre-wrap">
                      {selectedLog.error_message || "No error message available"}
                    </p>
                  </div>
                </div>
