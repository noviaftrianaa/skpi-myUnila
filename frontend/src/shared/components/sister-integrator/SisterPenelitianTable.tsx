...
export default function SisterPenelitianTable() {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const response = await sisterPenelitianService.getList({
          page: currentPage,
          limit: rowsPerPage,
        ...
        console.error('Error loading penelitian:', error);
      } finally {
        setLoading(false);
      }
    };
              ...
              {item.anggota.slice(0, 3).map((anggota, idx) => (
                <div key={idx} className="flex items-start gap-2">
                  <Chip
                    size="sm"
                    variant="flat"
                    color={anggota.jenis_peran.includes("Ketua") ? "primary" : "default"}
