...
interface NilaiPerkuliahanListItem {
  id_reg_pd: string;
  id_kls: string;
  nim: string | null;
...
interface FeederNilaiPerkuliahanTableProps {
  onFilterChange?: (filters: { id_semester?: string[]; id_prodi?: string; id_kelas?: string }) => void;
}

export default function FeederNilaiPerkuliahanTable({ onFilterChange }: FeederNilaiPerkuliahanTableProps) {
  const [data, setData] = useState<NilaiPerkuliahanListItem[]>([]);
  ...
  useEffect(() => {
    const loadOptions = async () => {
      try {
        ...
        const semesterRes = await feederClient.get("/nilai-perkuliahan/helper/semester");
        if (semesterRes.data.success) {
          setSemesterOptions(semesterRes.data.data || []);
        }

        // Load prodi list
        const prodiRes = await feederClient.get("/nilai-perkuliahan/helper/prodi");
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const response = await feederClient.get(`/nilai-perkuliahan?${params.toString()}`);
        ...
        console.error('Error loading nilai perkuliahan:', error);
        setData([]);
      } finally {
        setLoading(false);
      }
  ...
  const columns: Column<NilaiPerkuliahanListItem>[] = [
    {
      key: "nim",
      label: "NIM",
      sortable: true,
      ...
      render: (item) => (
        <div className="max-w-xs">
          ...
          <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
            {item.kode_matkul && <span className="mr-2">{item.kode_matkul}</span>}
