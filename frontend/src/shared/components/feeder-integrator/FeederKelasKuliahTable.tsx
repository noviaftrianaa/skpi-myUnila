...
interface FeederKelasKuliahTableProps {
  onFilterChange?: (filters: { id_semester?: string[]; id_prodi?: string }) => void;
}

export default function FeederKelasKuliahTable({ onFilterChange }: FeederKelasKuliahTableProps) {
  const [data, setData] = useState<KelasKuliahListItem[]>([]);
  ...
  useEffect(() => {
    const loadOptions = async () => {
      try {
        ...
        const semesterRes = await feederClient.get("/kelas-kuliah/helper/semester");
        if (semesterRes.data.success) {
          setSemesterOptions(semesterRes.data.data);
        }

        // Load prodi list
        const prodiRes = await feederClient.get("/kelas-kuliah/helper/prodi");
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const response = await feederClient.get(`/kelas-kuliah?${params.toString()}`);
        ...
        console.error('Error loading kelas kuliah:', error);
        setData([]);
      } finally {
        setLoading(false);
      }
  ...
  const columns: Column<KelasKuliahListItem>[] = [
    {
      key: "nm_kls",
      label: "NAMA KELAS",
      sortable: true,
      ...
      render: (item) => (
        <div className="max-w-xs">
          {item.nama_dosen ? (
            ...
            <span className="text-xs text-gray-400 italic">Belum ada dosen</span>
          )}
          {item.jumlah_dosen > 0 && (
            <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              {item.jumlah_dosen} orang
            </div>
          )}
        </div>
      ),
    ...
    >
      <motion.div variants={itemVariants}>
        <DataTable
          ...
          searchPlaceholder="Cari nama kelas atau mata kuliah..."
