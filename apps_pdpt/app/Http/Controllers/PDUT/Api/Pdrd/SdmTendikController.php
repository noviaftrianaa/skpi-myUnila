...
class SdmTendikController extends Controller
{
    ...
    public function detail(Request $request)
    {
        ...
        try {
            ...
            WHERE sdm.id_jns_sdm = 13 AND sdm.id_sdm = ? ", [$request->id_sdm]);

            if (empty($detail)) {
                return WrapResponse([], 'tidak ada detail tendik yang ditampilkan', FALSE);
            }
            ...
            return WrapResponse([$th], 'gagal mendapatkan detail tendik berdasarkan id_sdm', FALSE);
        }
        return WrapResponse(['data' => $data], 'detail tendik berdasarkan id_sdm', TRUE);
    }
