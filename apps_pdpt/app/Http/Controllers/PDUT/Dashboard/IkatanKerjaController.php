...
{
    private $reportName = 'Ikatan Kerja';
    private $title = '';
    ...
    public function index()
    {
        return view('dashboard.dosen.index', [
            ...
            'judul_layout'=> 'Ikatan Kerja Dosen',
            'side_active'   => 'dashboard.ikatan_kerja',
            'info'      =>  [
    ...
    public function chart(Request $request)
    {
        ...
        if($requestType!=='table')
        {
            ...
            {
                $where = " AND id_ikatan_kerja='".$selectedPointID."'";
            } elseif ($currentLevel=='Program Studi') {
                $where = " AND id_ikatan_kerja='".$lastLevelID."'";
            }

            $listCategories = DB::SELECT("
                SELECT id_ikatan_kerja as id,
                nm_ikatan_kerja as nama
                FROM ref.ikatan_kerja_sdm WITH (NOLOCK)
        ...
        $query_join     = " JOIN pdrd.reg_ptk treg WITH (NOLOCK) ON treg.id_sdm = tsdm.id_sdm AND treg.soft_delete=0
            ...
            JOIN ref.ikatan_kerja_sdm AS tikat ON tikat.id_ikatan_kerja=treg.id_ikatan_kerja
        ";
        ...
        /** GROUP BY */
        $query_group    = " GROUP BY tikat.id_ikatan_kerja, id_thn_ajaran ";
        ...
        if($currentLevel!=='Perguruan Tinggi' && $requestType!=='table')
        {
            if ($currentLevel=='Program Studi') {
                $query_where .= " AND tikat.id_ikatan_kerja='".$lastLevelID."' AND tfak.id_sms='".$selectedPointID."' ";
            } else {
                $query_where .= " AND tikat.id_ikatan_kerja = '".$selectedPointID."' ";
            }
        }
        ...
        if($requestType=='table')
        {
            ...
            /** Jika Kategori terplih adalah Dosen, maka tampilkan id_ikatan_kerja NULL */
            if ($currentLevel=='Perguruan Tinggi') {
                $query_where .= " AND tikat.id_ikatan_kerja='".$currentCategory."' ";
            } elseif ($currentLevel!='Perguruan Tinggi') {
                $query_where .= " AND tikat.id_ikatan_kerja='".$lastLevelID."'";
            }
    ...
    public function load()
    {
        ...
        $filename       = sha1('DosenIkatanKerja');
