...
{
    private $reportName = 'Status Keaktifan';
    private $title = '';
    ...
    public function index()
    {
        return view('dashboard.dosen.index', [
            ...
            'judul_layout'=> 'Status Keaktifan Dosen',
            'side_active'   => 'dashboard.status_keaktifan',
            'info'      =>  [
                'Menampilkan dosen berdasarkan status keaktifan pada tahun data diambil',
            ],
        ]);
    }
    ...
    public function chart(Request $request)
    {
        ...
        if($requestType!=='table')
        {
            ...
            {
                $where = " AND id_stat_aktif='".$selectedPointID."'";
            } elseif ($currentLevel=='Program Studi') {
                $where = " AND id_stat_aktif='".$lastLevelID."'";
            }

            $listCategories = DB::SELECT("
                SELECT id_stat_aktif as id,
                nm_stat_aktif as nama
                FROM ref.status_keaktifan_pegawai WITH (NOLOCK)
        ...
        $query_join     = " JOIN pdrd.reg_ptk treg WITH (NOLOCK) ON treg.id_sdm = tsdm.id_sdm AND treg.soft_delete=0
            ...
            JOIN ref.status_keaktifan_pegawai AS tst_aktif ON tst_aktif.id_stat_aktif=tsdm.id_stat_aktif
        ";
        ...
        /** GROUP BY */
        $query_group    = " GROUP BY tst_aktif.id_stat_aktif, id_thn_ajaran ";
        ...
        if($currentLevel!=='Perguruan Tinggi' && $requestType!=='table')
        {
            if ($currentLevel=='Program Studi') {
                $query_where .= " AND tst_aktif.id_stat_aktif='".$lastLevelID."' AND tfak.id_sms='".$selectedPointID."' ";
            } else {
                $query_where .= " AND tst_aktif.id_stat_aktif = '".$selectedPointID."' ";
            }
        }
        ...
        if($requestType=='table')
        {
            ...
            /** Jika Kategori terplih adalah Dosen, maka tampilkan id_stat_aktif NULL */
            if ($currentLevel=='Perguruan Tinggi') {
                $query_where .= " AND tst_aktif.id_stat_aktif='".$currentCategory."' ";
            } elseif ($currentLevel!='Perguruan Tinggi') {
                $query_where .= " AND tst_aktif.id_stat_aktif='".$lastLevelID."'";
            }
    ...
    public function load()
    {
        ...
        $filename       = sha1('DosenStatusKeaktifan');
