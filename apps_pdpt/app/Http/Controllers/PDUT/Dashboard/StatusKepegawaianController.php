...
{
    private $reportName = 'Status Kepegawaian';
    private $title = '';
    ...
    public function index()
    {
        return view('dashboard.dosen.index', [
            ...
            'judul_layout'=> 'Status Kepegawaian Dosen',
            'side_active'   => 'dashboard.status_kepegawaian',
            'info'      =>  [
    ...
    public function chart(Request $request)
    {
        ...
        if($requestType!=='table')
        {
            ...
            {
                $where = " AND id_stat_pegawai='".$selectedPointID."'";
            } elseif ($currentLevel=='Program Studi') {
                $where = " AND id_stat_pegawai='".$lastLevelID."'";
            }

            $listCategories = DB::SELECT("
                SELECT id_stat_pegawai as id,
                nm_stat_pegawai as nama
                FROM ref.status_kepegawaian WITH (NOLOCK)
        ...
        SELECT
            COUNT(*) as total,
            tstat_kepeg.id_stat_pegawai AS id,
        ...
        $query_join     = " JOIN pdrd.reg_ptk treg WITH (NOLOCK) ON treg.id_sdm = tsdm.id_sdm AND treg.soft_delete=0
            ...
            JOIN ref.status_kepegawaian AS tstat_kepeg ON tstat_kepeg.id_stat_pegawai=treg.id_stat_pegawai
        ";
        ...
        /** GROUP BY */
        $query_group    = " GROUP BY tstat_kepeg.id_stat_pegawai, id_thn_ajaran ";
        ...
        if($currentLevel!=='Perguruan Tinggi' && $requestType!=='table')
        {
            if ($currentLevel=='Program Studi') {
                $query_where .= " AND tstat_kepeg.id_stat_pegawai='".$lastLevelID."' AND tfak.id_sms='".$selectedPointID."' ";
            } else {
                $query_where .= " AND tstat_kepeg.id_stat_pegawai = '".$selectedPointID."' ";
            }
        }
        ...
        if($requestType=='table')
        {
            ...
            /** Jika Kategori terplih adalah Dosen, maka tampilkan id_stat_pegawai NULL */
            if ($currentLevel=='Perguruan Tinggi') {
                $query_where .= " AND tstat_kepeg.id_stat_pegawai='".$currentCategory."' ";
            } elseif ($currentLevel!='Perguruan Tinggi') {
                $query_where .= " AND tstat_kepeg.id_stat_pegawai='".$lastLevelID."'";
            }
    ...
    public function load()
    {
        ...
        $filename       = sha1('DosenStatusKepegawaian');
