...
<div class="sidebar">
    ...
    <div class="user-panel mt-3 pb-2 mb-2 d-flex text-light">
        <div class="image mt-1">
            <img src="{{ asset('images/blank-profile.png') }}" class="mt-2" alt="User Image">
        </div>
        <div class="info">
                ...
                <form action="{{ url('changeRole') }}" method="post" enctype="multipart/form-data" id="changeRole">
                    ...
                    <select class="form-control-plaintext bg-dark text-light peran" name="id_peran">
                        @php
                            ...
                            WHERE role.id_pengguna='" .
                                    auth()->user()->id_pengguna .
                                    "' AND role.soft_delete=0
                        ",
                            );
                        @endphp
                        @forelse ($peran as $items)
                            <option value="{{ $items->id_peran }}"
                                {{ $items->id_peran == session()->get('login.role')->id_peran ? 'selected' : '' }}>
                                {{ strtoupper($items->nm_peran) }}</option>
                        @empty
                        @endforelse
                    </select>
                </form>
    ...
    <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
            ...
            @if (!empty(\Session::get('login.menu')))
                ...
                @forelse (\Session::get('login.menu') as $n => $r)
                    @if (empty($r->sub))
                        @if (\Route::has($r->nm_file))
                            <li class="nav-item">
                        ...
                        @endif
                    @else
                        <li class="nav-item {{ AktifMenu($r->nm_file, 1) == 'active' ? 'menu-open' : '' }}" id="{{ $r->nm_file }}">
                            <a href="#" class="nav-link {{ AktifMenu($r->nm_file, 1) }}">
                                <i class="nav-icon {{ $r->icon }}"></i>
                                <p>
                                    {{ $r->nm_menu }}
                            ...
                            <ul class="nav nav-treeview">
                                @forelse ($r->sub as $t)
                                    @if (\Route::has($t->nm_file))
                                        <li class="nav-item {{ AktifMenu($t->nm_file, 2) == 'active' ? 'menu-open' : '' }}">
                                            <a href="{{ route($t->nm_file) }}"
                                                class="nav-link {{ AktifMenu($t->nm_file, 2) }}">
                                                <i class="{{ $t->icon }} nav-icon ml-4"></i>
                                                <p>{{ $t->nm_menu }}</p>
                                            </a>
                                        </li>
                                    @endif
