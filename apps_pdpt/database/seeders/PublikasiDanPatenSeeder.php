...
class PublikasiDanPatenSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $get_all_publikasi = \DB::connection('pgsql_sister')->SELECT("
            SELECT
                DISTINCT p.id_publikasi
        ...
        ");
        $get_page_publikasi = collect($get_all_publikasi)->chunk(1000)->toArray();
        $total_pub = count($get_all_publikasi);
        for($page_pub=0;$page_pub<count($get_page_publikasi);$page_pub++) {
            foreach ($get_page_publikasi[$page_pub] AS $urutan_pub=>$each_data_publikasi) {
                echo "Publikasi ".(($page_pub*1000)+$urutan_pub+1)." dari ".$total_pub." id:".$each_data_publikasi->id_publikasi;
                $cari_pub_od = Publikasi::find($each_data_publikasi->id_publikasi);
                if (is_null($cari_pub_od)) {
                    $cari_data_publikasi = \DB::connection('pgsql_sister')->table('pdrd.publikasi AS p')
                        ->where('p.id_publikasi',$each_data_publikasi->id_publikasi)
                        ->select([
                            'p.id_publikasi',
                            'p.id_jns_pub',
                            'p.judul',
                        ...
                        ->first();
                    $input_pub = (array) $cari_data_publikasi;
                    $input_pub['tgl_terbit']    = date('Y-m-d',strtotime($cari_data_publikasi['tgl_terbit']));
