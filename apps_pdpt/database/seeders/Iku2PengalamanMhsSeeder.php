    ...
    public function tempIku()
    {
        $query = DB::SELECT("
            SELECT
                ...
                CASE
                    WHEN d_mbkm.id_reg_pd = d_mbkm.id_reg_pd THEN 1
                END AS stat_kegiatan,
                ...
                END AS kat_kegiatan,
                CASE
                    WHEN d_mbkm.id_reg_pd = d_mbkm.id_reg_pd THEN 0
                END AS peringkat,
                sdm.nidn,
                sdm.nm_sdm AS nm_pembimbing,
                k1_mbkm.sks_mk AS total_sks,
                (
                    SELECT
                        CASE
                            WHEN k2_mbkm.sks_mk >= 20 THEN 1
                            ELSE 0
                        END AS status_iku
                    FROM
                        mbkm.konversi_kampus_merdeka AS k2_mbkm WITH(NOLOCK)
                    WHERE
                        k2_mbkm.id_daftar_kampus_merdeka = d_mbkm.id_daftar_kampus_merdeka
                        AND k2_mbkm.soft_delete = 0
                ) status_iku
            FROM
                ...
                JOIN mbkm.konversi_kampus_merdeka AS k1_mbkm ON k1_mbkm.id_daftar_kampus_merdeka = d_mbkm.id_daftar_kampus_merdeka
                AND k1_mbkm.soft_delete = 0
                ...
                LEFT JOIN pdrd.akt_mhs AS akt WITH(NOLOCK) ON akt.id_akt_mhs = k1_mbkm.id_akt_mhs
            ...
            SELECT
                ...
                CONCAT(sms.nm_lemb, ' (', jenjang.nm_jenj_didik, ')') AS nm_prodi,
                CASE
                    WHEN prestasi.id_pd = prestasi.id_pd THEN 2
                END AS stat_kegiatan,
                prestasi.nm_prestasi AS nm_kegiatan,
                akt.judul_akt_mhs AS nm_aktivitas,
                akt.lokasi_kegiatan,
                tkt_prestasi1.nm_tkt_prestasi AS kat_kegiatan,
                prestasi.peringkat,
                sdm.nidn,
                sdm.nm_sdm AS nm_pembimbing,
                CASE
                    WHEN prestasi.id_pd = prestasi.id_pd THEN 0
                END AS total_sks,
                (
                    SELECT
                        CASE
