...
###############################################################################
# Update Sister Service Environment Variables
# This script populates SISTER_* variables from existing DB values
###############################################################################
...
if [ -z "$SISTER_API_BASE_URL" ]; then
    ...
    log_error "Please set this value first"
    exit 1
fi
...
# Update SISTER_REDIS_HOST
if grep -q "^SISTER_REDIS_HOST=" "$ENV_FILE"; then
    sed -i "s|^SISTER_REDIS_HOST=.*|SISTER_REDIS_HOST=redis|" "$ENV_FILE"
    log_info "✓ Updated SISTER_REDIS_HOST=redis"
else
    echo "SISTER_REDIS_HOST=redis" >> "$ENV_FILE"
    log_info "✓ Added SISTER_REDIS_HOST=redis"
fi

# Update SISTER_REDIS_PORT
if grep -q "^SISTER_REDIS_PORT=" "$ENV_FILE"; then
    sed -i "s|^SISTER_REDIS_PORT=.*|SISTER_REDIS_PORT=6379|" "$ENV_FILE"
    log_info "✓ Updated SISTER_REDIS_PORT=6379"
else
    echo "SISTER_REDIS_PORT=6379" >> "$ENV_FILE"
    log_info "✓ Added SISTER_REDIS_PORT=6379"
fi
...
log_info ""
log_warn "Next steps:"
log_warn "  1. Review the changes: cat $ENV_FILE | grep -E 'SISTER_|API_CONFIG'"
log_warn "  2. Restart sister service:"
log_warn "     cd $DEPLOYMENT_DIR"
log_warn "     ./scripts/restart-sister-with-env.sh"
log_warn ""
