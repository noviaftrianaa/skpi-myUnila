...
show_menu() {
    ...
    echo ""
    echo -e "  ${BLUE}6)${NC} Restart All Services"
    echo -e "  ${BLUE}7)${NC} Restart Dashboard Only"
    echo -e "  ${BLUE}8)${NC} Restart Auth Only"
    echo -e "  ${BLUE}9)${NC} Restart Sister Only"
    echo ""
    echo -e "  ${YELLOW}10)${NC} Show Container Status"
    echo -e "  ${YELLOW}11)${NC} Show Logs"
    echo -e "  ${YELLOW}12)${NC} Test Endpoints"
    echo ""
...
show_logs() {
    ...
    case $log_choice in
        ...
        4) docker logs myunila-nginx --tail 100 -f ;;
        5) docker logs myunila-redis --tail 100 -f ;;
        6) docker logs myunila-meilisearch --tail 100 -f ;;
        *) echo "Invalid choice" ;;
    esac
}
