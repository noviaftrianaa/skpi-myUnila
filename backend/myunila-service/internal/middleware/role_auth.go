...
func RequireRole(allowedRoles ...string) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if err != nil {
			...
			return c.Status(fiber.StatusForbidden).JSON(fiber.Map{
				"success": false,
				"message": fmt.Sprintf("Insufficient permissions. Required role: %v", allowedRoles),
			})
		}
		...
		return c.Status(fiber.StatusForbidden).JSON(fiber.Map{
			"success": false,
			"message": fmt.Sprintf("Insufficient permissions. Your active role '%s' is not allowed. Required: %v", activeContext.NmPeran, allowedRoles),
		})
	}
}
...
func getActiveContext(userID string) (*ActiveContext, error) {
	...
	// Get from Redis using global client
	data, err := database.RedisGet(ctx, cacheKey)
	if err != nil {
		return nil, err
	}
