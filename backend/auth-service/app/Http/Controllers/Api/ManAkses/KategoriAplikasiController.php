<?php

namespace App\Http\Controllers\Api\ManAkses;

use App\Http\Controllers\Controller;
use App\Services\ManAkses\KategoriAplikasiService;
use App\Repositories\ManAkses\KategoriAplikasiRepository;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;

/**
 * Kategori Aplikasi Controller
 * API endpoints for kategori aplikasi management
 */
class KategoriAplikasiController extends Controller
{
    protected KategoriAplikasiService $service;

    public function __construct()
    {
        $repository = new KategoriAplikasiRepository();
        $this->service = new KategoriAplikasiService($repository);
    }

    /**
     * Get all categories (for dropdown)
     *
     * @return JsonResponse
     */
    public function all(): JsonResponse
    {
        try {
            $result = $this->service->getAll();

            return response()->json([
                'success' => true,
                'message' => 'Data kategori berhasil diambil',
                'data' => $result
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Gagal mengambil data kategori: ' . $e->getMessage(),
                'data' => null
            ], 500);
        }
    }

    /**
     * Get paginated list of categories
     *
     * @param Request $request
     * @return JsonResponse
     */
    public function index(Request $request): JsonResponse
    {
        try {
            $params = [
                'page' => (int) $request->get('page', 1),
                'limit' => (int) $request->get('limit', 10),
                'search' => $request->get('search'),
                'sort_by' => $request->get('sort_by'), // column name
                'sort_order' => $request->get('sort_order'), // 'asc', 'desc'
            ];

            $result = $this->service->getList($params);

            return response()->json([
                'success' => true,
                'message' => 'Data kategori berhasil diambil',
                'data' => $result
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Gagal mengambil data kategori: ' . $e->getMessage(),
                'data' => null
            ], 500);
        }
    }

    /**
     * Get category detail
     *
     * @param string $id
     * @return JsonResponse
     */
    public function show(string $id): JsonResponse
    {
        try {
            $result = $this->service->getDetail($id);

            if (!$result) {
                return response()->json([
                    'success' => false,
                    'message' => 'Kategori tidak ditemukan',
                    'data' => null
                ], 404);
            }

            return response()->json([
                'success' => true,
                'message' => 'Detail kategori berhasil diambil',
                'data' => $result
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Gagal mengambil detail kategori: ' . $e->getMessage(),
                'data' => null
            ], 500);
        }
    }

    /**
     * Get category statistics
     *
     * @return JsonResponse
     */
    public function stats(): JsonResponse
    {
        try {
            $result = $this->service->getStats();

            return response()->json([
                'success' => true,
                'message' => 'Statistik kategori berhasil diambil',
                'data' => $result
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Gagal mengambil statistik kategori: ' . $e->getMessage(),
                'data' => null
            ], 500);
        }
    }

    /**
     * Create new category
     *
     * @param Request $request
     * @return JsonResponse
     */
    public function store(Request $request): JsonResponse
    {
        try {
            $request->validate([
                'nm_kategori' => 'required|string|max:100',
                'icon_kategori' => 'nullable|string|max:100',
                'icon_color' => 'nullable|string|max:50',
                'urutan' => 'nullable|integer',
                'a_aktif' => 'nullable|boolean',
            ]);

            $data = $request->only([
                'nm_kategori', 'icon_kategori', 'icon_color', 'urutan', 'a_aktif'
            ]);

            $result = $this->service->create($data);

            return response()->json([
                'success' => true,
                'message' => 'Kategori berhasil ditambahkan',
                'data' => $result
            ], 201);
        } catch (\Illuminate\Validation\ValidationException $e) {
            return response()->json([
                'success' => false,
                'message' => 'Validasi gagal',
                'errors' => $e->errors()
            ], 422);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Gagal menambahkan kategori: ' . $e->getMessage(),
                'data' => null
            ], 500);
        }
    }

    /**
     * Update existing category
     *
     * @param Request $request
     * @param string $id
     * @return JsonResponse
     */
    public function update(Request $request, string $id): JsonResponse
    {
        try {
            $request->validate([
                'nm_kategori' => 'required|string|max:100',
                'icon_kategori' => 'nullable|string|max:100',
                'icon_color' => 'nullable|string|max:50',
                'urutan' => 'nullable|integer',
                'a_aktif' => 'nullable|boolean',
            ]);

            $data = $request->only([
                'nm_kategori', 'icon_kategori', 'icon_color', 'urutan', 'a_aktif'
            ]);

            $result = $this->service->update($id, $data);

            if (!$result) {
                return response()->json([
                    'success' => false,
                    'message' => 'Kategori tidak ditemukan',
                    'data' => null
                ], 404);
            }

            return response()->json([
                'success' => true,
                'message' => 'Kategori berhasil diperbarui',
                'data' => $result
            ]);
        } catch (\Illuminate\Validation\ValidationException $e) {
            return response()->json([
                'success' => false,
                'message' => 'Validasi gagal',
                'errors' => $e->errors()
            ], 422);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Gagal memperbarui kategori: ' . $e->getMessage(),
                'data' => null
            ], 500);
        }
    }

    /**
     * Delete category (soft delete)
     *
     * @param string $id
     * @return JsonResponse
     */
    public function destroy(string $id): JsonResponse
    {
        try {
            $result = $this->service->delete($id);

            if (!$result) {
                return response()->json([
                    'success' => false,
                    'message' => 'Kategori tidak ditemukan',
                    'data' => null
                ], 404);
            }

            return response()->json([
                'success' => true,
                'message' => 'Kategori berhasil dihapus',
                'data' => null
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Gagal menghapus kategori: ' . $e->getMessage(),
                'data' => null
            ], 500);
        }
    }
}
