...
// SetService sets the global apiconfig service instance
func SetService(s Service) {
	globalService = s
}

// GetService returns the global apiconfig service instance
func GetService() Service {
	return globalService
}

// GetByCode is a convenience method that uses the global service
func GetByCode(apiCode string) (*APIConfigWithCredentials, error) {
	if globalService == nil {
		return nil, fmt.Errorf("apiconfig service not initialized")
	}
	
	dto, err := globalService.GetByAPICode(apiCode)
	if err != nil {
		return nil, err
	}
	...
	credentials, err := globalService.GetCredentials(apiCode)
	if err != nil {
		return nil, err
	}
	
	// Convert DTO back to APIConfig
	config := &APIConfig{
		ID:              dto.ID,
		APICode:         dto.APICode,
		APIName:         dto.APIName,
		...
		UseEnvFallback:  dto.UseEnvFallback,
		CreatedAt:       dto.CreatedAt,
		UpdatedAt:       dto.UpdatedAt,
	}
	
	if dto.APIDescription != "" {
		config.APIDescription = sql.NullString{String: dto.APIDescription, Valid: true}
	}
	if dto.AdditionalHeaders != "" {
		config.AdditionalHeaders = sql.NullString{String: dto.AdditionalHeaders, Valid: true}
	}
	
	return &APIConfigWithCredentials{
		APIConfig:   *config,
