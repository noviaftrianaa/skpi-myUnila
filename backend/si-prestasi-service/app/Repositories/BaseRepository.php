...
{
    // =========================================
    // PostgreSQL (si_prestasi) - READ/WRITE
    // =========================================
    ...
    public function pgBeginTransaction(?string $userId = null, ?string $ipAddress = null): void
    {
        ...
        if ($userId) {
            ...
            DB::connection('pgsql')->unprepared("SET LOCAL siprestasi.id_pengguna = '{$safeUserId}'");
        }
        if ($ipAddress) {
            $safeIp = addslashes($ipAddress);
            DB::connection('pgsql')->unprepared("SET LOCAL siprestasi.ip_address = '{$safeIp}'");
        }
    }
