...
/**
 * SertifikasiRepository — CRUD untuk prestasi.sertifikasi.
 * Pattern sama dengan PrestasiMandiriRepository, tapi tanpa kategori/peringkat/kelompok/bentuk.
 */
class SertifikasiRepository extends BaseRepository
{
    private const PARENT_TIPE = 'SERTIFIKASI';
    ...
    public function list(array $filters, int $page = 1, int $limit = 20): array
    {
        ...
        if (!empty($filters['status_workflow'])) {
            $where[] = 's.status_workflow = ?';
            $bindings[] = $filters['status_workflow'];
        }
        if (!empty($filters['search'])) {
            $where[] = '(s.nm_sertifikasi ILIKE ? OR s.nm_penyelenggara ILIKE ?)';
            $q = '%' . $filters['search'] . '%';
            $bindings[] = $q;
            $bindings[] = $q;
        }
        ...
        $totalRow = $this->pgSelectOne(
            "SELECT COUNT(*) AS total FROM prestasi.sertifikasi s WHERE {$whereSql}",
            $bindings
        );
        ...
        $rows = $this->pgSelect(
            "SELECT
                ...
                (SELECT COUNT(*) FROM prestasi.peserta_mhs pmh
                    WHERE pmh.id_parent = s.id_sertifikasi AND pmh.parent_tipe = ?) AS jumlah_peserta_mhs,
                (SELECT COUNT(*) FROM prestasi.peserta_dosen pds
                    WHERE pds.id_parent = s.id_sertifikasi AND pds.parent_tipe = ?) AS jumlah_peserta_dosen
            FROM prestasi.sertifikasi s
            LEFT JOIN ref.level_prestasi lvl ON lvl.id_level_prestasi = s.id_level_prestasi
            WHERE {$whereSql}
    ...
    public function create(array $data, array $pesertaMhs, array $pesertaDosen, ?string $userId = null, ?string $ip = null): string
    {
        ...
        try {
            $row = $this->pgInsertReturning("
                INSERT INTO prestasi.sertifikasi (
                    kode_pt, thn_prestasi, id_level_prestasi,
                    nm_sertifikasi, nm_penyelenggara,
    ...
    public function update(string $id, array $data, array $pesertaMhs, array $pesertaDosen, ?string $userId = null, ?string $ip = null): bool
    {
        ...
        try {
            $affected = $this->pgUpdate("
                UPDATE prestasi.sertifikasi SET
                    kode_pt = ?,
                    thn_prestasi = ?,
                    id_level_prestasi = ?,
                    nm_sertifikasi = ?,
                    ...
                    id_updater = ?,
                    updated_at = now()
                WHERE id_sertifikasi = ? AND soft_delete = false
            ", [
                $data['kode_pt'] ?? null,
                (int) $data['thn_prestasi'],
                $data['id_level_prestasi'],
