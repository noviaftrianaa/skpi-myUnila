...
class AkademikDataRepository extends BaseDataRepository
{
    ...
    public function getProdiStats(): array
    {
        ...
        // KONSISTEN dengan BerandaRepository: filter stat_prodi='A' (prodi aktif)
        $row = $this->select("
            SELECT
                COUNT(*) AS total,
        ...
        // Breakdown akreditasi (current akreditasi aktif tiap prodi)
    ...
    public function getAkreditasiStats(): array
    {
        $row = $this->select("
            SELECT
                ...
                SUM(CASE WHEN ap.tst_sk_akreditasi_prodi BETWEEN GETDATE() AND DATEADD(DAY, 90, GETDATE()) THEN 1 ELSE 0 END) AS akan_expire,
