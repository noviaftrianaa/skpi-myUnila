...
abstract class BaseRepository
{
    ...
    public function pgBeginTransaction(?string $userId = null, ?string $ipAddress = null): void
    {
        ...
        if ($userId) {
            ...
            DB::connection('pgsql')->unprepared("SET LOCAL siknila.id_pengguna = '{$safeUserId}'");
        }
        if ($ipAddress) {
            $safeIp = addslashes($ipAddress);
            DB::connection('pgsql')->unprepared("SET LOCAL siknila.ip_address = '{$safeIp}'");
        }
    }
